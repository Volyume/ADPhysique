...
export default function CoachBuilderScreen({ navigation, route }) {
  ...
  // Auto-generate the plan immediately if all fields came prefilled from ProOnboarding.
  // The empty dependency array is intentional — runs once on mount only.
  useEffect(() => {
    if (isFullyPrefilled && !hasAutoGeneratedRef.current) {
