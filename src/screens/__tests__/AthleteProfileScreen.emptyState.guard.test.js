...
describe('AthleteProfileScreen shared empty state', () => {
  test('uses EmptyState for the strength baselines no-data state', () => {
    ...
    expect(source).toMatch(
      /<EmptyState[\s\S]*icon="barbell-outline"[\s\S]*title="Strength standards unlock with data"[\s\S]*text="Log body weight and your main lifts to compare your strength against baseline standards\."[\s\S]*compact/,
    );
