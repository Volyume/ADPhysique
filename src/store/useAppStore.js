...
// launch (subscriptions audit C-3/H1). Without this the cached tier='pro' was
// trusted offline / before the cloud read with no comparison to the trial end,
// so a just-expired trial user kept Pro until the next successful cloud read.
// Only the trial path is enforced locally; paid_pro lapses are server/RTDN-driven.
...
const useAppStore = create((set, get) => ({
  ...
  checkTier: async () => {
    try {
      ...
      // grant was removed (subscriptions audit M-3): it could grant Pro on a
      // cleared tier cache, and the cloud read is the real source of truth.
      if (tier === 'pro') {
