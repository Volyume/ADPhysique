...
const NOTIF_ID_RECOVERY_WEEK = 'volyume_recovery_week_started';
const RECOVERY_WEEK_NOTIFIED_KEY = (userId, mesocycleId) =>
  `@volyume_recovery_week_notified_${userId}_${mesocycleId}`;

/**
 * ONE notification when the planned recovery week actually starts.
 * (C18 recovery-visibility amendment, the OPTIONAL half.)
 *
 * FROM THE REAL TRANSITION, NEVER THE CALENDAR. The caller passes the block's
 * already-resolved recovery state; this fires only when that state is the
 * block's own planned recovery week. Nothing here reads a weekday, and there
 * is no "it is Monday so send it".
 *
 * ONCE PER BLOCK. A stored marker keyed by the block means re-entering Home
 * every day of the recovery week cannot send a second push, and the marker
 * dies with the block rather than with a dismissal.
 *
 * NEVER FOR THE ADAPTIVE STATE. A mid-block recovery adjustment is carried by
 * Home and the next-workout surface. Notifying on every softening is the
 * nagging the amendment forbids, and the athlete sees it before their next
 * session anyway.
 *
 * IN-APP IS NOT DEPENDENT ON THIS. With notifications off entirely, Home,
 * next-workout and Train still say everything. This is supplemental.
 *
 * Opt-out, permission, quiet hours and the push budget are all inherited by
 * going through exactly the path the block-ready push uses.
 */
export async function notifyRecoveryWeekStarted(userId, recoveryState, mesocycleId) {
  if (Platform.OS === 'web') return;
  try {
    // eslint-disable-next-line global-require
    const { RECOVERY_STATE, RECOVERY_WEEK_NOTIFICATION } = require('../recoveryState');
    if (!userId || !mesocycleId) return;
    if (recoveryState?.state !== RECOVERY_STATE.PLANNED_BLOCK_RECOVERY) return;

    const key = RECOVERY_WEEK_NOTIFIED_KEY(userId, mesocycleId);
    // Fails CLOSED on a read error: an unreadable marker is treated as
    // already-sent, because a missing notification is a smaller harm than a
    // repeated one.
    let already = true;
    try { already = !!(await AsyncStorage.getItem(key)); } catch (_) { already = true; }
    if (already) return;

    // eslint-disable-next-line global-require
    const { isCategoryEnabled } = require('./categoryPrefs');
    if (!(await isCategoryEnabled(CATEGORY.WEEKLY_COACH_READY))) return;
    ...
    await Notifications.scheduleNotificationAsync({
      identifier: NOTIF_ID_RECOVERY_WEEK,
      content: {
        title: RECOVERY_WEEK_NOTIFICATION.title,
        body: RECOVERY_WEEK_NOTIFICATION.body,
        data: { type: 'recovery_week_started' },
        sound: false,
      },
      trigger: {
        channelId: COACHING_REMINDERS_CHANNEL,
    ...
    // Marked only AFTER it is genuinely laid, so a refused budget slot or a
    // quiet-hours stand-down does not silently consume the one notification.
