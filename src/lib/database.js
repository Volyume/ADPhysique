...
// ─── Custom exercises (locked split, IDENTITY_AND_OWNERSHIP_LOCKED.md) ────────
//
// Per-user exercise rows live here with composite PK (user_id, id).
// The legacy `exercises` table stays library-only (NULL user_id). UI
// surfaces that want "all exercises for the current user" union both
// via getAllExercisesForUser below.

export async function insertCustomExercise(userId, data) {
  if (!userId) throw new Error('insertCustomExercise requires userId');
  const d = await db();
  ...
  await d.runAsync(
    `INSERT INTO custom_exercises
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id, userId, data.name, data.primaryMuscle ?? null,
      data.secondaryMuscles ? JSON.stringify(data.secondaryMuscles) : null,
      data.equipment ?? null, data.movementPattern ?? null,
      data.compoundIsolation ?? null, data.defaultRepMin ?? null,
      data.defaultRepMax ?? null, data.fatigueCost ?? null,
      data.stimulusToFatigueRatio ?? null, data.subregion ?? null,
      data.exerciseCategory ?? 'compound', data.incrementKg ?? 2.5,
...
export async function updateCustomExercise(userId, id, patch) {
  if (!userId || !id) return false;
  const d = await db();
  const fields = [];
  const values = [];
  const map = {
    name: 'name', primaryMuscle: 'primary_muscle',
    equipment: 'equipment', movementPattern: 'movement_pattern',
    compoundIsolation: 'compound_isolation',
    defaultRepMin: 'default_rep_min', defaultRepMax: 'default_rep_max',
    fatigueCost: 'fatigue_cost', stimulusToFatigueRatio: 'stimulus_to_fatigue_ratio',
    subregion: 'subregion', exerciseCategory: 'exercise_category',
    incrementKg: 'increment_kg', notes: 'notes',
  };
  for (const [key, col] of Object.entries(map)) {
    if (key in patch) { fields.push(`${col} = ?`); values.push(patch[key]); }
  }
  if ('secondaryMuscles' in patch) {
    fields.push('secondary_muscles = ?');
    values.push(patch.secondaryMuscles ? JSON.stringify(patch.secondaryMuscles) : null);
  }
  if (fields.length === 0) return false;
  fields.push('updated_at = ?');
  values.push(Date.now(), userId, id);
  await d.runAsync(
    `UPDATE custom_exercises SET ${fields.join(', ')}
...
export async function getCustomExercisesForUser(userId) {
  if (!userId) return [];
  const d = await db();
  const rows = await d.getAllAsync(
  ...
  return rows.map(r => ({ ...rowToCamel(r), isCustom: true }));
}

export async function getCustomExerciseById(userId, id) {
  if (!userId || !id) return null;
  const d = await db();
  const row = await d.getFirstAsync(
    `SELECT * FROM custom_exercises
     WHERE user_id = ? AND id = ? AND deleted_at IS NULL LIMIT 1`,
    [userId, id],
  );
  return row ? { ...rowToCamel(row), isCustom: true } : null;
}

/**
 * UNION of the global library + this user's customs. UI that shows
 * "every exercise available to the user" calls this; the legacy
 * getAllExercises() keeps returning library-only and the customs come
 * out of getCustomExercisesForUser. Both are async + cheap.
 */
export async function getAllExercisesForUser(userId) {
  const [library, customs] = await Promise.all([
    getAllExercises(),
    userId ? getCustomExercisesForUser(userId) : Promise.resolve([]),
  ]);
  // Customs first so a custom with the same name as a library entry
  // wins in pickers (matches the existing isCustom display priority).
  return [...customs, ...library];
}

// Delta fetcher used by the sync layer. Returns custom_exercises
// rows whose updated_at is newer than the timestamp the caller
// supplies. Includes soft-deleted rows so cloud knows to tombstone.
export async function getAllCustomExercisesSince(userId, sinceMs = 0) {
  if (!userId) return [];
  const d = await db();
  const rows = await d.getAllAsync(
    `SELECT * FROM custom_exercises
...
// Cloud applier called by pullFromCloud when custom_exercises rows
// arrive. Last-write-wins per (user_id, id).
export async function insertOrUpdateCustomExerciseFromCloud(userId, row) {
  if (!userId || !row?.id) return;
  const d = await db();
  const toMs = (t) => (typeof t === 'string' ? new Date(t).getTime() : (t ?? Date.now()));
  await d.runAsync(
    `INSERT INTO custom_exercises
      (id, user_id, name, primary_muscle, secondary_muscles, equipment,
       movement_pattern, compound_isolation, default_rep_min, default_rep_max,
       fatigue_cost, stimulus_to_fatigue_ratio, subregion, exercise_category,
       increment_kg, notes, created_at, updated_at, deleted_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(user_id, id) DO UPDATE SET
       ...
       stimulus_to_fatigue_ratio = excluded.stimulus_to_fatigue_ratio,
    ...
    [
      ...
      typeof row.secondary_muscles === 'string'
        ...
        : (row.secondary_muscles ? JSON.stringify(row.secondary_muscles) : null),
      row.equipment ?? null, row.movement_pattern ?? null,
      row.compound_isolation ?? null, row.default_rep_min ?? null,
      row.default_rep_max ?? null, row.fatigue_cost ?? null,
      row.stimulus_to_fatigue_ratio ?? null, row.subregion ?? null,
      row.exercise_category ?? 'compound', row.increment_kg ?? 2.5,
      row.notes ?? null,
      toMs(row.created_at), toMs(row.updated_at),
      row.deleted_at ? toMs(row.deleted_at) : null,
    ],
  );
}
