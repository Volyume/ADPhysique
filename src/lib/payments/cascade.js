...
// C-2 client safety net (subscriptions audit). The authoritative revocation of
// a cancelled / lapsed / refunded PAID subscription is the Google Play RTDN
// Pub/Sub push (play-billing-rtdn → upgrade_tier_for_user 'free'). Until that
// push subscription is wired in the Play/GCP console, the server never learns a
// paid sub ended, so the tier stays 'pro'. This checks Play directly on launch
// for a paid_pro user and downgrades if Google reports no active 'pro'
// entitlement.
//
...
// A false revoke (Play momentarily empty) is recoverable by the user via
// Restore purchases; a missed revoke just waits for the next launch.
export async function reconcilePaidEntitlement(profile) {
  try {
    ...
    // Play definitively reports no active Pro entitlement for a paid_pro user:
    // the subscription has lapsed / been cancelled past its period / refunded.
