/**
 * band.js — generated by scripts/exercise-library/convert-legacy.mjs
 * (EL-14, docs/exercise-library-expansion-2026-09-05/07-CORPUS-FORMAT.md
 * section 1-2). 23 entries, converted one-to-one from the legacy
 * seedExercises.js RAW tuple (plus seedRoutines.js REQUIRED_EXERCISES,
 * EL-15) with names UNCHANGED (canonical id is a hash of the name).
 *
 * Conventions: alphabetical within each movement section; pure object
 * literals, no imports except from ./vocab.js; cue is '' until a later
 * agent authors it (EL-17); a retired entry ({ name, retiredInto }) marks
 * an EL-21 duplicate-consolidation, kept so its canonical id stays
 * resolvable for the id-remap top-up.
 *
 * Sources policy: every row here already shipped in production; this is
 * a format migration, not new content. New rows added in a later stage of
 * this campaign follow EL-3's quality gate and are attested against
 * coaching/exercise-science literature or the open datasets checklist.
 */

export default [
  {
    name: "Band Assisted Pull-Up",
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "band",
    movementPattern: "pull",
    compound: true,
    repMin: 8, repMax: 15,
    fatigueCost: 2, sfr: 4,
    subregion: "vertical_pull",
    aliases: ["Assisted Pull-Up (Band)"],
    loadCharacter: "grind",
    setup: "Loop a band around the bar and place a knee or foot inside it.",
    execution: "Pull yourself up until your chin clears the bar, then lower under control.",
    watch: "Letting the band do all the work instead of pulling actively defeats the point.",
  },
  {
    name: "Band Bicep Curl",
    primaryMuscle: "biceps",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "short_head",
    aliases: ["Band Curl","resistance band seated biceps curl"],
    loadCharacter: "grind",
    setup: "Stand on the band, feet shoulder width apart, palms facing forward.",
    execution: "Curl your hands towards your shoulders with elbows pinned to your sides, then lower slowly.",
    watch: "Swinging the elbows forward turns it into a shoulder movement.",
  },
  {
    name: "Band Chest Press",
    primaryMuscle: "chest",
    secondaryMuscles: ["triceps","front_delts"],
    equipment: "band",
    movementPattern: "push",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "flat",
    aliases: ["Legend Chest Press","resistance band seated chest press"],
    loadCharacter: "grind",
    setup: "Anchor the band behind you at chest height, standing facing away, arms bent with tension in the band.",
    execution: "Press your hands forward to full extension, then return under control.",
    watch: "Letting the shoulders round forward takes tension off the chest.",
  },
  {
    name: "Band Deadlift",
    primaryMuscle: "hamstrings",
    secondaryMuscles: ["glutes","back"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 10, repMax: 15,
    fatigueCost: 2, sfr: 4,
    subregion: "hip_extension",
    loadCharacter: "grind",
    setup: "Stand on the band, feet hip width apart, holding the handles by your thighs.",
    execution: "Push your hips back and lower the handles down your shins, then drive forward to standing.",
    watch: "Rounding the lower back instead of hinging is the main fault.",
  },
  {
    name: "Band Face Pull",
    primaryMuscle: "rear_delts",
    secondaryMuscles: ["back","traps"],
    equipment: "band",
    movementPattern: "pull",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: "face_pull",
    aliases: ["Face pulls with yellow/green band"],
    loadCharacter: "grind",
    setup: "Anchor the band at head height and hold it with both hands, arms extended in front.",
    execution: "Pull it towards your face leading with your elbows so they finish wide.",
    watch: "Pulling with straight arms misses the rear shoulders.",
  },
  {
    name: "Band Good Morning",
    primaryMuscle: "hamstrings",
    secondaryMuscles: ["glutes","back"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 10, repMax: 15,
    fatigueCost: 2, sfr: 4,
    subregion: "hip_extension",
    aliases: ["Band Good Morning (Pull Through)"],
    loadCharacter: "grind",
    setup: "Stand on the band with it looped over your shoulders, feet hip width apart.",
    execution: "Hinge forward from the hips with a soft bend in the knees, then return to standing.",
    watch: "Bending the knees too much turns the movement into a squat.",
  },
  {
    name: "Band Hip Thrust",
    primaryMuscle: "glutes",
    secondaryMuscles: ["hamstrings"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "activator",
    aliases: ["band hip lift","Hip Extension with Bands","Hip Lift with Band"],
    loadCharacter: "grind",
    setup: "Lie on your back with the band across your hips, feet flat on the floor.",
    execution: "Drive your hips up by squeezing your glutes, then lower under control.",
    watch: "Arching the lower back for extra height is the common fault.",
  },
  {
    name: "Band Lat Pulldown",
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "band",
    movementPattern: "pull",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "vertical_pull",
    aliases: ["band underhand pulldown"],
    loadCharacter: "grind",
    setup: "Anchor the band overhead, kneeling and facing it, holding the ends above your head.",
    execution: "Pull your hands down towards your chest leading with your elbows, then let it return slowly.",
    watch: "Pulling with the arms alone reduces the effect.",
  },
  {
    name: "Band Lateral Raise",
    primaryMuscle: "side_delts",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: null,
    loadCharacter: "grind",
    setup: "Stand on the band holding a handle in each hand by your sides.",
    execution: "Raise your arms out to shoulder height with a slight bend in the elbows, then lower slowly.",
    watch: "Shrugging the shoulders up towards the ears takes work off the side of the shoulder.",
  },
  {
    name: "Band Leg Curl",
    primaryMuscle: "hamstrings",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "knee_flexion",
    aliases: ["Leg Curl","Leg curl with elastic"],
    loadCharacter: "grind",
    setup: "Anchor the band low behind you and loop it around one ankle, standing tall.",
    execution: "Bend that knee to curl your heel towards your glute, then lower with control.",
    watch: "Letting the hips sway hides how little work is happening.",
  },
  {
    name: "Band Overhead Tricep Extension",
    primaryMuscle: "triceps",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "overhead",
    aliases: ["band side triceps extension","Speed Band Overhead Triceps"],
    loadCharacter: "grind",
    setup: "Anchor the band low and hold it with both hands overhead, elbows pointing forward.",
    execution: "Straighten your arms fully, then bend to lower the band behind your head.",
    watch: "Letting the elbows wing out to the sides takes tension off the triceps.",
  },
  {
    name: "Band Pull-Apart",
    primaryMuscle: "rear_delts",
    secondaryMuscles: ["back"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: "horiz_abduction",
    aliases: ["Banded Scapular Retraction"],
    loadCharacter: "grind",
    setup: "Hold the band with both hands out in front at shoulder height.",
    execution: "Pull it apart by drawing your hands out to the sides until it reaches your chest.",
    watch: "Bending the elbows to help pull it apart reduces the work on the rear shoulders.",
  },
  {
    name: "Band Pull-Through",
    primaryMuscle: "hamstrings",
    secondaryMuscles: ["glutes"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "hip_extension",
    loadCharacter: "grind",
    setup: "Anchor the band low behind you and stand facing away with it running between your legs.",
    execution: "Hinge forward slightly, then drive your hips forward, squeezing your glutes at the top.",
    watch: "Standing too far forward loses the hip hinge that makes it work.",
  },
  {
    name: "Band Row",
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "band",
    movementPattern: "pull",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "upper_mid_row",
    loadCharacter: "grind",
    setup: "Anchor the band at chest height and step back until there is tension.",
    execution: "Pull your hands towards your ribs, squeezing your shoulder blades together, then return slowly.",
    watch: "Rounding the shoulders forward reduces the stretch on the back.",
  },
  {
    name: "Band Shoulder Press",
    primaryMuscle: "front_delts",
    secondaryMuscles: ["triceps","side_delts"],
    equipment: "band",
    movementPattern: "push",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: null,
    aliases: ["Band Overhead Press"],
    loadCharacter: "grind",
    setup: "Stand on the band with a handle at each shoulder, palms facing forward.",
    execution: "Press both hands overhead to full extension, then lower back to the shoulders.",
    watch: "Arching the lower back to help the press up takes the work off the shoulders.",
  },
  {
    name: "Band Squat",
    primaryMuscle: "quads",
    secondaryMuscles: ["glutes"],
    equipment: "band",
    movementPattern: "squat",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "squat_press",
    loadCharacter: "grind",
    setup: "Stand on the band, feet shoulder width apart, holding the handles at your shoulders.",
    execution: "Bend your knees and hips to lower down, then drive back up to standing.",
    watch: "Letting the knees cave inward as you stand is the main fault to watch for.",
  },
  {
    name: "Band Tibialis Raise",
    primaryMuscle: "tibialis",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: null,
    aliases: ["band single leg reverse calf raise"],
    loadCharacter: "grind",
    setup: "Anchor the band low in front and loop it around one foot, standing on the other leg.",
    execution: "Pull your toes up towards your shin against the band, then lower slowly.",
    watch: "Rushing the lowering reduces the effect.",
  },
  {
    name: "Band Tricep Pushdown",
    primaryMuscle: "triceps",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "pushdown",
    aliases: ["Band Pushdown","Tricep Pull-Down"],
    loadCharacter: "grind",
    setup: "Anchor the band overhead, holding the ends with elbows tucked at your sides.",
    execution: "Straighten your arms down fully, then let them bend back up under control.",
    watch: "Letting the elbows drift forward turns it into a chest movement.",
  },
  {
    name: "Band Wrist Curl",
    primaryMuscle: "forearms",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 3,
    subregion: null,
    loadCharacter: "grind",
    setup: "Hold the band under one foot, gripping the other end palm up, forearm resting on your thigh.",
    execution: "Curl your wrist upward, then lower it slowly under control.",
    watch: "Moving the whole forearm instead of just the wrist reduces the effect.",
  },
  {
    name: "Band Wrist Extension",
    primaryMuscle: "forearms",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 3,
    subregion: null,
    loadCharacter: "grind",
    setup: "Hold the band under one foot, gripping the end palm down, forearm resting on your thigh.",
    execution: "Lift your knuckles upward against the band, then lower slowly.",
    watch: "Letting the elbow lift off the thigh takes the work away from the wrist.",
  },
  {
    name: "Banded Lateral Walk",
    primaryMuscle: "glutes",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: "pumper",
    aliases: ["Band Lateral Walk","Monster Walk (Band)","Lateral Walk"],
    loadCharacter: "grind",
    setup: "Loop a band around your ankles or above your knees and bend into a slight squat.",
    execution: "Step sideways keeping tension in the band, then bring the trailing foot in without slack.",
    watch: "Standing tall between steps lets the band lose its tension.",
  },
  {
    name: "Seated Band Lat Pulldown",
    primaryMuscle: "back",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "pull",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "vertical_pull",
    loadCharacter: "grind",
    setup: "Sit tall with the band anchored overhead, gripped wide in both hands.",
    execution: "Pull it down towards your upper chest by driving your elbows down and back, then let it return with control.",
    watch: "Leaning back to use bodyweight is the common shortcut.",
  },
  {
    name: "Seated Band Row",
    primaryMuscle: "back",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "pull",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "horizontal_lat",
    // EL-25 (exercise-library-expansion-2026-09-05): "Band Seated Row"
    // (never shipped) re-created this row under a folded name; dropped
    // outright rather than retired, and its aliases carried over here.
    aliases: ["Band Seated Row", "resistance band seated straight back row"],
    loadCharacter: "grind",
    setup: "Sit with legs extended and the band looped around your feet, an end in each hand.",
    execution: "Pull your elbows back past your ribs while keeping your chest tall, then extend forward.",
    watch: "Rounding the back to add range loses tension in the target muscles.",
  },
  // ── INTEGRATION STAGE 2 (exercise-library-expansion-2026-09-05) — generated by scripts/exercise-library/integrate-inventories.mjs; rerun the script to regenerate, do not hand-edit below this line ──
  {
    name: "Band Ankle Dorsiflexion",
    primaryMuscle: "tibialis",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 30,
    fatigueCost: 1, sfr: 5,
    subregion: null,
    aliases: ["Exercise Band Dorsiflexion"],
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral","demands":{"position":"seated","floorAccess":true,"overheadPosition":false,"gripDemand":"none","bilateralUpper":false,"unilateralLoadable":true,"bilateralLower":false,"balanceDemand":"supported","axialLoad":false,"weightBearingHands":false}},
    setup: "Sit with the band looped around the top of one foot, anchored in front of you.",
    execution: "Pull your toes up towards your shin against the band, then release slowly.",
    watch: "Using the whole leg to yank the band cuts the ankle out of it.",
  },
  {
    name: "Band Ankle Plantarflexion",
    primaryMuscle: "calves",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 5,
    subregion: "gastro",
    aliases: ["Seated Band Calf Raise","Band Plantar Flexion"],
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral","demands":{"position":"seated","floorAccess":false,"overheadPosition":false,"gripDemand":"none","unilateralLoadable":true,"bilateralUpper":false,"bilateralLower":false,"axialLoad":false,"impact":false,"balanceDemand":"supported","weightBearingHands":false}},
    setup: "Sit with legs extended, the band looped around the ball of one foot, held taut in your hands.",
    execution: "Point your toes away from you against the band, then return with control.",
    watch: "Bending the knee to help the push takes work off the calf.",
  },
  {
    name: "Band Assisted Ab Wheel Rollout",
    primaryMuscle: "abs",
    secondaryMuscles: ["front_delts","back"],
    equipment: "band",
    movementPattern: "core",
    compound: true,
    repMin: 8, repMax: 15,
    fatigueCost: 3, sfr: 4,
    subregion: "anti_extension",
    aliases: ["Band-Assisted Ab Rollout"],
    loadCharacter: "grind",
    overrides: {"difficulty":2,"demands":{"position":"kneeling","floorAccess":true,"overheadPosition":false,"gripDemand":"bar","unilateralLoadable":false,"bilateralUpper":true,"bilateralLower":false,"axialLoad":false,"impact":false,"balanceDemand":"high","weightBearingHands":true}},
    setup: "Kneel and loop a band over a high anchor and around the ab wheel to reduce the load.",
    execution: "Roll the wheel forward keeping hips level and back flat, then pull back to kneeling.",
    watch: "Letting the hips sag towards the floor is the main fault.",
  },
  {
    name: "Band Assisted Dip",
    primaryMuscle: "triceps",
    secondaryMuscles: ["chest","front_delts"],
    equipment: "band",
    movementPattern: "push",
    compound: true,
    repMin: 6, repMax: 15,
    fatigueCost: 2, sfr: 4,
    subregion: "pushdown",
    loadCharacter: "grind",
    overrides: {"demands":{"position":"mixed","floorAccess":false,"overheadPosition":false,"gripDemand":"bar","bilateralUpper":true,"unilateralLoadable":false,"balanceDemand":"supported","axialLoad":false,"weightBearingHands":false}},
    setup: "Loop a band around the dip bars and place a knee or foot in the loop for support.",
    execution: "Lower your body by bending your elbows, then press back up to full lock out.",
    watch: "Leaning too far forward shifts work from triceps towards the chest.",
  },
  {
    name: "Band Chest Fly (Standing)",
    primaryMuscle: "chest",
    secondaryMuscles: ["front_delts"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "flat",
    aliases: ["Cross Over - With Bands"],
    loadCharacter: "grind",
    setup: "Anchor the band behind you at chest height, an end in each hand, arms out to the sides with a slight elbow bend.",
    execution: "Bring your hands together in front of your chest in a wide arc, then return slowly.",
    watch: "More elbow bend turns it into a press.",
  },
  {
    name: "Band Chest Press (Single-Arm)",
    primaryMuscle: "chest",
    secondaryMuscles: ["triceps","front_delts"],
    equipment: "band",
    movementPattern: "push",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "flat",
    aliases: ["band one arm twisting chest press"],
    loadCharacter: "grind",
    overrides: {"difficulty":2,"demands":{"unilateralLoadable":true,"bilateralUpper":false,"axialLoad":false}},
    setup: "Anchor the band behind you at chest height, standing in a staggered stance holding one end at chest level.",
    execution: "Press your hand forward until the arm is straight, then return with control.",
    watch: "Twisting the torso to help takes work off the chest.",
  },
  {
    name: "Band Clamshell",
    primaryMuscle: "glutes",
    secondaryMuscles: ["adductors"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 5,
    subregion: "pumper",
    aliases: ["Banded Clamshell"],
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral","demands":{"position":"lying","floorAccess":true,"overheadPosition":false,"gripDemand":"none","bilateralUpper":false,"unilateralLoadable":true,"bilateralLower":false,"balanceDemand":"supported","axialLoad":false,"weightBearingHands":false}},
    setup: "Lie on your side with knees bent and a band looped around your thighs, feet stacked together.",
    execution: "Keeping your feet touching, lift your top knee towards the ceiling, then lower it back.",
    watch: "Rolling your hips backward loses the position fastest.",
  },
  {
    name: "Band Concentration Curl",
    primaryMuscle: "biceps",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 10, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "short_head",
    loadCharacter: "grind",
    overrides: {"demands":{"position":"seated","unilateralLoadable":true,"bilateralUpper":false}},
    setup: "Sit with the band anchored under your foot, elbow braced against the inside of your thigh.",
    execution: "Curl your hand up towards your shoulder, then lower fully under control.",
    watch: "Letting the elbow drift off the thigh brings the shoulder into the lift.",
  },
  {
    name: "Band Fire Hydrant (Kneeling)",
    primaryMuscle: "glutes",
    secondaryMuscles: ["adductors"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: "pumper",
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral","demands":{"position":"kneeling","floorAccess":true,"overheadPosition":false,"gripDemand":"none","bilateralUpper":false,"unilateralLoadable":true,"bilateralLower":false,"balanceDemand":"supported","axialLoad":false,"weightBearingHands":true}},
    setup: "Kneel on all fours with a band looped around your thighs just above the knees.",
    execution: "Lift one knee out to the side keeping your hips square, then lower it back.",
    watch: "Letting the hips rotate open is the main compensation to watch.",
  },
  {
    name: "Band Front Raise",
    primaryMuscle: "front_delts",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: null,
    loadCharacter: "grind",
    setup: "Stand on the band, feet hip-width apart, an end in each hand in front of your thighs.",
    execution: "Raise your arms straight in front to shoulder height, then lower with control.",
    watch: "Swinging from the hips to launch the band takes work off the shoulders.",
  },
  {
    name: "Band Glute Bridge",
    primaryMuscle: "glutes",
    secondaryMuscles: ["hamstrings"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "activator",
    loadCharacter: "grind",
    setup: "Lie on your back with knees bent and a band looped around your thighs, feet flat on the floor.",
    execution: "Drive your hips up while pressing your knees out against the band, squeezing at the top.",
    watch: "Knees collapsing inward is the fault to expose.",
  },
  {
    name: "Band Glute Kickback",
    primaryMuscle: "glutes",
    secondaryMuscles: ["hamstrings"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 5,
    subregion: "stretcher",
    loadCharacter: "grind",
    overrides: {"demands":{"position":"kneeling","floorAccess":true,"overheadPosition":false,"gripDemand":"none","bilateralUpper":false,"unilateralLoadable":true,"bilateralLower":false,"balanceDemand":"supported","axialLoad":false,"weightBearingHands":true}},
    setup: "Get on all fours with the band looped around one foot, anchored under the other knee or a fixed point.",
    execution: "Extend the banded leg straight back and up without arching your back, then return.",
    watch: "Arching the spine for height loses glute work.",
  },
  {
    name: "Band Goblet Squat",
    primaryMuscle: "quads",
    secondaryMuscles: ["glutes"],
    equipment: "band",
    movementPattern: "squat",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "squat_press",
    loadCharacter: "grind",
    setup: "Stand on the centre of the band, feet shoulder-width apart, both ends held at chest height like a goblet.",
    execution: "Sit your hips back and down into a squat, chest tall, then drive back up.",
    watch: "The band pulling your shoulders forward rounds the back.",
  },
  {
    name: "Band Hammer Curl",
    primaryMuscle: "biceps",
    secondaryMuscles: ["forearms"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "brachialis",
    loadCharacter: "grind",
    setup: "Stand on the band, feet hip-width apart, an end in each hand with palms facing your body.",
    execution: "Curl your hands up towards your shoulders keeping your palms in, then lower fully.",
    watch: "Swinging the elbows forward turns the curl into a shoulder move.",
  },
  {
    name: "Band Hip Thrust (Single-Leg)",
    primaryMuscle: "glutes",
    secondaryMuscles: ["hamstrings"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 5,
    subregion: "activator",
    loadCharacter: "grind",
    overrides: {"difficulty":2,"demands":{"balanceDemand":"supported"}},
    setup: "Sit with your upper back against a bench, a band anchored low behind you and looped around your hips.",
    execution: "Drive through one heel to lift your hips, other leg lifted, then lower with control.",
    watch: "Twisting towards the working leg is the common fault.",
  },
  {
    name: "Band Incline Press",
    primaryMuscle: "chest",
    secondaryMuscles: ["triceps","front_delts"],
    equipment: "band",
    movementPattern: "push",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "incline",
    loadCharacter: "grind",
    overrides: {"difficulty":2},
    setup: "Anchor the band low behind you, an end in each hand at shoulder height, staggered stance leaning slightly forward.",
    execution: "Press your hands up and forward at an incline angle, then return slowly.",
    watch: "Arching the back for range takes work off the chest.",
  },
  {
    name: "Band Leg Extension",
    primaryMuscle: "quads",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "knee_extension",
    aliases: ["Resistance Band Leg Extension"],
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral","demands":{"position":"seated","floorAccess":false,"overheadPosition":false,"gripDemand":"none","unilateralLoadable":true,"bilateralUpper":false,"bilateralLower":false,"axialLoad":false,"impact":false,"balanceDemand":"supported","weightBearingHands":false}},
    setup: "Sit tall with the band anchored behind you and looped around one ankle.",
    execution: "Extend that knee straight out in front against the band, then lower with control.",
    watch: "Leaning back to use momentum takes the work away from the quad.",
  },
  {
    name: "Band Monster Walk",
    primaryMuscle: "glutes",
    secondaryMuscles: ["quads"],
    equipment: "band",
    movementPattern: "isolation",
    compound: true,
    repMin: 15, repMax: 30,
    fatigueCost: 2, sfr: 4,
    subregion: "pumper",
    loadCharacter: "grind",
    overrides: {"laterality":"alternating","demands":{"gripDemand":"none","weightBearingHands":false,"axialLoad":false}},
    setup: "Loop a band around your thighs or ankles and drop into a quarter squat, knees pressed slightly out.",
    execution: "Step sideways in small steps keeping tension on the band, then repeat the other way.",
    watch: "Letting the knees drift inward loses the point of it.",
  },
  {
    name: "Band Pallof Press",
    primaryMuscle: "abs",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 10, repMax: 15,
    fatigueCost: 1, sfr: 5,
    subregion: "rotation",
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral","demands":{"position":"standing"}},
    setup: "Stand side on to the anchor with the band at chest height, both hands together in front of your sternum.",
    execution: "Press your hands out away from your chest without rotating the torso towards the anchor, then return.",
    watch: "Twisting is what it resists.",
  },
  {
    name: "Band Pull-Apart (Overhead)",
    primaryMuscle: "rear_delts",
    secondaryMuscles: ["back"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: "horiz_abduction",
    loadCharacter: "grind",
    overrides: {"demands":{"overheadPosition":true}},
    setup: "Hold the band overhead, hands wider than shoulder width, arms straight.",
    execution: "Pull the band apart by driving your hands out and slightly down, squeezing your shoulder blades, then return.",
    watch: "Bending the elbows turns it into a pulldown.",
  },
  {
    name: "Band Rear Delt Fly",
    primaryMuscle: "rear_delts",
    secondaryMuscles: ["back"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "horiz_abduction",
    aliases: ["band reverse fly","Back Flyes - With Bands"],
    loadCharacter: "grind",
    setup: "Anchor the band in front of you at chest height, an end in each hand, arms extended.",
    execution: "Pull your hands out and back in a wide arc, squeezing your shoulder blades, then return slowly.",
    watch: "Bending the elbows too much shifts the work elsewhere.",
  },
  {
    name: "Band Romanian Deadlift (Bilateral)",
    primaryMuscle: "hamstrings",
    secondaryMuscles: ["glutes"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "hip_extension",
    aliases: ["band straight back stiff leg deadlift"],
    loadCharacter: "grind",
    setup: "Stand on the band, feet hip-width apart, an end in each hand in front of your thighs.",
    execution: "Push your hips back and lower the band down your legs with a soft knee bend, then drive forward to stand.",
    watch: "Rounding the lower back is the main fault.",
  },
  {
    name: "Band Romanian Deadlift (Single-Leg)",
    primaryMuscle: "hamstrings",
    secondaryMuscles: ["glutes"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 8, repMax: 15,
    fatigueCost: 2, sfr: 4,
    subregion: "hip_extension",
    loadCharacter: "grind",
    overrides: {"difficulty":2,"demands":{"balanceDemand":"high","axialLoad":false}},
    setup: "Stand on the band with one foot, an end in each hand, weight shifted onto that leg.",
    execution: "Hinge forward at the hip while the other leg extends back for balance, then return to standing.",
    watch: "Letting the hips rotate open signals losing control.",
  },
  {
    name: "Band Row (Single-Arm)",
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "band",
    movementPattern: "pull",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "upper_mid_row",
    aliases: ["band one arm standing low row","band one arm twisting seated row"],
    loadCharacter: "grind",
    overrides: {"demands":{"unilateralLoadable":true,"bilateralUpper":false,"axialLoad":false}},
    setup: "Anchor the band at chest height and stand side on, holding one end in a staggered stance.",
    execution: "Pull your elbow back past your ribs keeping your torso still, then extend forward.",
    watch: "Rotating the torso to help takes tension off the back.",
  },
  // EL-25 (exercise-library-expansion-2026-09-05): "Band Seated Row" was
  // never shipped and duplicated "Seated Band Row" (a folded-name
  // word-order collision) above under a different name. Deleted outright
  // (never retired into a stub, since it never shipped) and added as an
  // alias of "Seated Band Row"; its own alias carried over there too.
  {
    name: "Band Shoulder Press (Seated)",
    primaryMuscle: "front_delts",
    secondaryMuscles: ["triceps"],
    equipment: "band",
    movementPattern: "push",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "overhead_press",
    loadCharacter: "grind",
    overrides: {"demands":{"position":"seated","axialLoad":false}},
    setup: "Sit tall on a chair or bench, band anchored under the seat, an end in each hand at shoulder height.",
    execution: "Press hands straight overhead, then lower back to shoulder height.",
    watch: "Arching the back for height takes work off the shoulders.",
  },
  {
    name: "Band Standing Curl (Single-Arm)",
    primaryMuscle: "biceps",
    secondaryMuscles: ["forearms"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 10, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "short_head",
    aliases: ["band one arm overhead biceps curl"],
    loadCharacter: "grind",
    overrides: {"demands":{"unilateralLoadable":true,"bilateralUpper":false}},
    setup: "Stand on the band with one foot, holding the free end in the same-side hand, arm extended.",
    execution: "Curl your hand up towards your shoulder without swinging the elbow forward, then lower fully.",
    watch: "Leaning back shifts work away from the biceps.",
  },
  {
    name: "Band Standing Hip Abduction",
    primaryMuscle: "glutes",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: "pumper",
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral","demands":{"gripDemand":"supportive","weightBearingHands":false,"unilateralLoadable":true}},
    setup: "Stand side on to the anchor, band around your ankle, a wall or support nearby for balance.",
    execution: "Lift the banded leg straight out to the side without leaning your torso, then lower with control.",
    watch: "Leaning away lets momentum do the work.",
  },
  {
    name: "Band Standing Hip Adduction",
    primaryMuscle: "adductors",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 15, repMax: 25,
    fatigueCost: 1, sfr: 4,
    subregion: null,
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral","demands":{"gripDemand":"supportive","weightBearingHands":false,"unilateralLoadable":true}},
    setup: "Stand side on to the anchor, band around your ankle, anchor on the same side as the working leg.",
    execution: "Pull the leg across your body towards the midline, then return under control.",
    watch: "Rotating the hips to help takes tension off the inner thigh.",
  },
  {
    name: "Band Straight-Arm Pulldown",
    primaryMuscle: "back",
    secondaryMuscles: ["triceps"],
    equipment: "band",
    movementPattern: "pull",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "vertical_pull",
    loadCharacter: "grind",
    overrides: {"demands":{"overheadPosition":true}},
    setup: "Stand facing a high anchor, holding the band with arms extended in front at shoulder height.",
    execution: "Pull your hands down towards your thighs keeping your elbows straight, then let it return slowly.",
    watch: "Bending the elbows turns it into a pushdown.",
  },
  {
    name: "Band Sumo Deadlift",
    primaryMuscle: "glutes",
    secondaryMuscles: ["hamstrings","adductors"],
    equipment: "band",
    movementPattern: "hinge",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "stretcher",
    loadCharacter: "grind",
    setup: "Stand on the band, feet wide and toes turned out, an end in each hand between your legs.",
    execution: "Push your hips back and bend your knees into the wide stance, then drive through your heels to stand tall.",
    watch: "Knees caving inward is the fault.",
  },
  {
    name: "Band T-Raise",
    primaryMuscle: "rear_delts",
    secondaryMuscles: ["traps"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "horiz_abduction",
    loadCharacter: "grind",
    overrides: {"demands":{"position":"standing","floorAccess":false,"balanceDemand":"stable"}},
    setup: "Hinge forward with a flat back, band anchored in front of you, an end in each hand.",
    execution: "Raise your arms out to the sides until level with your shoulders, forming a T, then lower with control.",
    watch: "Heaving with the back takes work off the shoulders.",
  },
  {
    name: "Band Tricep Kickback",
    primaryMuscle: "triceps",
    secondaryMuscles: [],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "pushdown",
    loadCharacter: "grind",
    overrides: {"demands":{"position":"standing","unilateralLoadable":true,"bilateralUpper":false}},
    setup: "Hinge forward with a flat back, upper arm close to your body and elbow bent, band anchored behind you.",
    execution: "Straighten your elbow to extend your hand back, then return under control.",
    watch: "Swinging the upper arm turns it into a shoulder movement.",
  },
  {
    name: "Band Upright Row",
    primaryMuscle: "side_delts",
    secondaryMuscles: ["traps"],
    equipment: "band",
    movementPattern: "pull",
    compound: true,
    repMin: 10, repMax: 20,
    fatigueCost: 2, sfr: 3,
    subregion: null,
    loadCharacter: "grind",
    overrides: {"difficulty":2,"demands":{"axialLoad":false}},
    setup: "Stand on the band, feet hip-width apart, an end in each hand in front of your thighs.",
    execution: "Pull your hands up towards your collarbone leading with your elbows, then lower with control.",
    watch: "Shrugging up for height takes work off the target muscles.",
  },
  {
    name: "Band W-Raise",
    primaryMuscle: "rear_delts",
    secondaryMuscles: ["traps"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: "horiz_abduction",
    loadCharacter: "grind",
    setup: "Stand on the band, an end in each hand at your sides.",
    execution: "Pull your hands up and out into a W shape by bending your elbows and squeezing your shoulder blades, then return.",
    watch: "Dropping the elbows below shoulder height turns the W into a shrug.",
  },
  {
    name: "Band Woodchop (High-to-Low)",
    primaryMuscle: "abs",
    secondaryMuscles: ["back"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "rotation",
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral"},
    setup: "Anchor the band up high and stand side on, both hands together near the anchor.",
    execution: "Pull the band down and across your body towards your opposite hip, rotating through the torso, then return slowly.",
    watch: "Moving only the arms skips the core.",
  },
  {
    name: "Band Woodchop (Low-to-High)",
    primaryMuscle: "abs",
    secondaryMuscles: ["back"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 2, sfr: 4,
    subregion: "rotation",
    loadCharacter: "grind",
    overrides: {"laterality":"unilateral"},
    setup: "Anchor the band down low and stand side on, both hands together near the anchor.",
    execution: "Pull the band up and across your body towards the opposite shoulder, rotating through the torso, then return slowly.",
    watch: "Arms alone leave the core out of it.",
  },
  {
    name: "Band Y-Raise",
    primaryMuscle: "front_delts",
    secondaryMuscles: ["traps"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 12, repMax: 20,
    fatigueCost: 1, sfr: 4,
    subregion: null,
    loadCharacter: "grind",
    overrides: {"demands":{"overheadPosition":true}},
    setup: "Stand on the band, feet hip-width apart, an end in each hand in front of your thighs.",
    execution: "Raise your arms up and out into a Y shape above your head, then lower with control.",
    watch: "Arching the lower back for height takes work off the shoulders.",
  },
  {
    name: "Band Y-T-W Raise",
    primaryMuscle: "rear_delts",
    secondaryMuscles: ["traps","back"],
    equipment: "band",
    movementPattern: "isolation",
    compound: false,
    repMin: 8, repMax: 15,
    fatigueCost: 2, sfr: 4,
    subregion: "horiz_abduction",
    loadCharacter: "grind",
    overrides: {"difficulty":2,"demands":{"overheadPosition":true,"position":"standing","floorAccess":false,"balanceDemand":"stable"}},
    setup: "Stand on the band or anchor it in front of you, an end in each hand.",
    execution: "Move through a Y, then a T, then a W shape, keeping your torso still throughout.",
    watch: "Using momentum to swing through each position takes the work off the shoulders and back.",
  },
  {
    name: "Push-Up (Band-Resisted)",
    primaryMuscle: "chest",
    secondaryMuscles: ["triceps","front_delts"],
    equipment: "band",
    movementPattern: "push",
    compound: true,
    repMin: 8, repMax: 15,
    fatigueCost: 3, sfr: 3,
    subregion: "flat",
    aliases: ["band push sit-up"],
    loadCharacter: "grind",
    overrides: {"difficulty":2,"demands":{"position":"lying","floorAccess":true,"overheadPosition":false,"gripDemand":"none","bilateralUpper":true,"unilateralLoadable":false,"balanceDemand":"supported","axialLoad":false,"weightBearingHands":true}},
    setup: "Loop a band across your upper back and under your hands in a push-up position.",
    execution: "Lower your chest to the floor as normal, then press up hard against the extra tension near the top.",
    watch: "Letting the band pull your shoulders forward rounds the back.",
  },
  // ── END INTEGRATION STAGE 2 ──
];
