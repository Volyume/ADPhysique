...
describe('partner accountability copy', () => {
  ...
  test('starts with a calm aim when the user has not set one', () => {
    ...
    expect(plan.headline).toBe('Set your weekly session number. Sam sees the number only, never your plan.');
    expect(plan.primaryAction).toMatchObject({ key: 'set_aim', label: 'Set weekly number' });
  ...
  test('keeps the privacy language narrow', () => {
    ...
    expect(copy).toContain('Only weekly training status, your weekly number, fixed cheers and approved win cards are shared.');
