...
describe('resolveIntention', () => {
  test('both aims set and equal -> one co-owned line, no per-person lines', () => {
    ...
    expect(r.shared).toBe('You both set training-week aims.');
  ...
  test('aims differ -> each own aim on its own line, no shared line', () => {
    ...
    expect(r.mine).toBe('You set your training-week aim.');
    expect(r.theirs).toBe('Sam set their training-week aim too.'); // ED-safe: partner's differing number withheld
  });

  test('only one side set -> only that own line', () => {
    expect(resolveIntention({ myAim: 5, partnerAim: 0 }).mine).toBe('You set your training-week aim.');
    ...
    expect(t.theirs).toBe('Sam set their training-week aim too.');
  });
  ...
  test('falls back to a warm partner name', () => {
    expect(resolveIntention({ myAim: 0, partnerAim: 3 }).theirs).toBe('Your partner set their training-week aim too.');
  });
