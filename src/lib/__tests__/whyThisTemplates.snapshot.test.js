...
describe('whyThisTemplates: locked output snapshots', () => {
  ...
  test('getAutoRegMessage all actions', () => {
    ...
    expect(getAutoRegMessage('deload_now', 5)).toMatchInlineSnapshot(`"Good timing: you've been building for 5 weeks. Next week is lighter: shorter sessions, same exercises, fewer sets."`);
    expect(getAutoRegMessage('deload_now', 2)).toMatchInlineSnapshot(`"Your recovery is dropping. Next week is lighter: shorter sessions, same exercises, fewer sets."`);
  });
