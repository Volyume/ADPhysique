...
describe('T-PROGRAMME-07: next-block recommendation branches (real getBlockAdvice, IO-mocked reads only)', () => {
  ...
  // The suite-level clock spy pins blockAdvisor's Date.now() to NOW, so this
  // integration fixture must use that same clock. Mixing a module-load live
  // timestamp with the later fixed spy makes the recovery block appear active.
