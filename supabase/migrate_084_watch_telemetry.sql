-- Migration 084: add the COMP-020 Apple Watch events to record_engine_telemetry.
--
-- COMP-020 (Apple Watch set-logging companion). Four events power adoption + the
-- reliability bar the whole design is scoped to protect:
--
--   watch_session_attached        a watch joined this session (adoption)
--   watch_set_logged              a set logged from the wrist (usage depth)
--   watch_apply_duplicate_dropped a replayed event was dropped (idempotency proof)
--   watch_replay_recovered        an event arrived via the durable queue after a
--                                 disconnect (recovered > 0, lost = 0, always)
--
-- Counts/flags only, NEVER training content. Flows through the same opt-out gate
-- as every other event. Emitters: src/lib/watch/bridge.js (the first three;
-- watch_replay_recovered is allow-listed ready for when the native channel tags
-- recovered events).
--
...
-- the row re-pushes forever. Reproduces the migration 081 list verbatim plus the
-- four new names.
--
...
-- Rollback:         re-apply migration 081 to drop the four names.
