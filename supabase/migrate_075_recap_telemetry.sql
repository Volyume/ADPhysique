-- Migration 075: add the COMP-005 recap-open event to record_engine_telemetry.
--
-- COMP-005 (Free Monthly Recap + block-end recap). One event measures recap
-- open rate:
--
--   recap_opened   (RecapStory mount, month/block variants)
--     payload: { variant } — month | block
--
-- No PII: a variant label only, never training or body data. Flows through the
...
-- the row re-pushes forever. Reproduces the migration 074 list verbatim plus
-- the one new name.
--
-- Applied locally:  NO (pending)
-- Applied remotely: NO (pending founder apply)
-- Safe to re-run:   YES. CREATE OR REPLACE; no schema change.
-- Rollback:         re-apply migration 074 to drop the name.
