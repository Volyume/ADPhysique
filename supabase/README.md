...
- **170 WRITTEN, NOT APPLIED (Community connection, part A; founder gate).**
  ...
  Carries: `community_profiles.discipline_keys` (15-key taxonomy, max 3)
  plus its validating/labelling helpers; `_community_profile_card` gains
  discipline fields and the nine migrate_165 consistency counters (gated on
  the owner's `share_consistency`/`status`/`is_minor`); `community_dimension`
  and `community_dimensions_me` (DROPped and re-created with a new required
  `_today` param) gain `discipline`/`age_band` cohorts with member/trained-
  today counts and a sample, via new helper `_community_cohort_stats`;
  `community_board` gains scopes `area`/`style`/`discipline`/`age_band`; new
  `community_hub_summary(_today)` returns cohorts+groups in one call;
  `community_find_people` (DROPped and re-created) gains a hard
  `_discipline` filter. No new table, no change to `delete_user_data()`.
  DEPENDS ON 160-165, 167, 169. Not yet applied; waits for the founder's
  exact phrase "run against production". Part B (ambient sharing, Together,
  Respect everyone, the digest) is a separate migration, not this file.
