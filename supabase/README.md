...
| 180 | `migrate_180_community_consistency_server_gate.sql` | Founder order 2026-09-22, item 6 ("safety backstop"; audit B-02/Question 5): the calm/ED withhold on consistency sharing was enforced client-only. New helper `_community_ed_flag_open(uuid)` (fail-closed true on a null id or any error) covers the open ED-pattern flag; calm mode is client-only for this purpose (fact (b), file header) and is not gated server-side. **DORMANT UNTIL D92-11 (Opus review 2026-09-23, H1, lead-verified): nothing writes the cloud `ed_pattern_flags` table today (the engine raises flags into device SQLite only; the sync registry has the table pull-only; no migration or edge function inserts it), so this gate, like the two edge functions' existing ED gates, withholds nothing for an engine-raised flag until the raise-only push recorded as D92-11 is decided by the founder. Audit B-02 is NOT closed by this file alone.** The acceptance block probes the helper (NULL true, no-row false, open-row true) and prints the open-flag count. Six readers re-issued byte-for-byte with the marked change (`_community_profile_card`, `community_board`, `_community_cohort_stats`, `community_hub_summary`, `community_group_get`, `community_friends_trained_today`); `community_update_training_profile` re-issued to store `share_consistency = false` when the caller's flag is open, response shape unchanged. No table changes. LEAD RULING 2026-09-23 (resolving the 176/180 conflict raised in the prior draft): `community_hub_summary` is re-issued from migrate_176's body (lines 88-241, which already carries 176's `AND g.status = 'active'` closed-groups change), not migrate_170's, so applying 180 alone or after 176 lands both fixes; 176 must never (re-)apply AFTER 180 unmodified, or it reverts the ED-gate (file header, Part 5). DEPENDS ON 017, 160, 165, 170, 171, 172, 176 (REQUIRED APPLY ORDER: 176 BEFORE 180, enforced in code: 180's Part 0 refuses until 176's `community_group_list_mine` change is live, and 176's Part 0 refuses to re-run once 180's gate is live). Rollback: see the file header. | **WRITTEN 2026-09-23, NOT APPLIED** (waits for the founder's phrase). |
