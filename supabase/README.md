...
| 175 | `migrate_175_community_rules_gate_tolerant.sql` | D160: the rules gate in `community_upsert_profile` tolerates an older client (the version accepted is stored; the acts that need the current text stay gated by `_community_require_rules`) and a client one version ahead; `_community_rules_version()` back to 3 in the same transaction. Re-issue of 170's function with four marked changes, guard-proved. DEPENDS ON 160, 161, 170, 174. Rollback: re-issue from 170 and 174/161 (file header). | **WRITTEN, NOT APPLIED** (founder order 2026-09-12; connector; checksum protocol). |
