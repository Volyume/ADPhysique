...
| 150 | `migrate_150_capability_telemetry.sql` | CC32 section 29 operational counters join `record_engine_telemetry`'s allow-list (no-candidate count, state-unavailable, unknown-axis hit, effective-diff applied/declined). Content-free by law (R1 #12). Until it runs the server rejects the five events and the client's fire-and-forget posts drop harmlessly. | **NO - NOT APPLIED.** Written 2026-08-21 (CC32). Independent of 145-149; apply in the same founder-gated batch. |
