...
- **163 WRITTEN, NOT APPLIED (Community place + finder; founder gate).**
...
- **173 WRITTEN, NOT APPLIED (Community at onboarding, CR-15 / D158: `community_handle_suggestion()`, the server-side handle suggestion the onboarding step and the Join screen pre-fill; founder phrase "run against production" NOT yet given; Claude-run through the connector under the checksum protocol on that phrase). GATED on founder question Q2 (data minimisation: the handle IS the sanitised email local part; if the answer is "given name only", Source 1 is removed before the phrase). Hostile-reviewed (Opus, 2026-09-11): four FIX FIRST items landed (a NULL-proof search_path check, a NULL-proof sanitiser case, the honest header sentence, four guard gaps closed); HOLDS on security, determinism, bounds and data flow. ORDER: apply before the next build, since the client half calls the RPC on the onboarding step and the Join screen and a missing function is logged as an unexpected transport error until it exists.** Written 2026-09-11 from `docs/communities-revamp-2026-09-10/25-ONBOARDING-COMMUNITY-SPEC.md` section 4.1. Three functions, no table changes: `_community_handle_suggest_reserved()` (the client's 80 reserved words, byte-for-byte, no client grant), `_community_handle_base(text)` (the pure sanitiser: lower, plus-tag dropped, non-alphanumeric runs to one underscore, trimmed, cut to twenty, NULL under three characters, invalid or excluded; no client grant), and the RPC (VOLATILE for its rate rail, 30 an hour; an existing member gets their own handle back; sources in order: the email local part read from `auth.users` inside the function and never returned, unless an Apple private relay; the provider's given name; `athlete`; collisions `_2`..`_99` then four random digits; at most 120 tries). `_community_handle_reserved()` is deliberately NOT widened (the upsert re-validates the merged handle on every save, so existing members with a now-reserved handle would lose every edit). Guard: `src/__tests__/migrate173.rpcOnly.guard.test.js`. The client half lands first and treats a missing function as `unavailable`. Rollback: three DROP FUNCTIONs (file header).
...
| 173 | `migrate_173_community_handle_suggestion.sql` | Community at onboarding (CR-15, D158): `community_handle_suggestion()` proposes a free handle for the caller, derived server-side from the email local part (never returned), the provider's given name, or `athlete`, with a bounded collision walk; two pure helpers (the sanitiser and the client's reserved-word list as an exclusion list, no client grants). VOLATILE (rate rail 30 an hour), SECURITY DEFINER, pinned search_path, EXECUTE to authenticated only. No table changes. DEPENDS ON 160, 170. Rollback: three DROP FUNCTIONs (file header). | **WRITTEN, NOT APPLIED** (awaits the founder's phrase; connector; checksum protocol). |
