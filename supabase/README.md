...
| 176 | `migrate_176_community_closed_groups_out_of_lists.sql` | CR-16 / D162: a closed group leaves the Hub's GROUPS section and "My groups" (`community_hub_summary`, `community_group_list_mine` keep `g.status = 'active'`), and an accepted invite returns the count after the join (`community_group_accept_invite` re-reads the group). Three CREATE OR REPLACE FUNCTIONs re-issued byte-for-byte with one marked change each, guard-proved; no table or grant changes beyond the re-stated pairs. DEPENDS ON 160, 165, 170. Rollback: re-issue from 170 and 165 (file header). | **WRITTEN 2026-09-13, NOT APPLIED** (waits for the founder's phrase). |
