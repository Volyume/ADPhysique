...
| 166 | `migrate_166_community_rate_check_hotfix.sql` | **PRODUCTION INCIDENT HOTFIX.** `_community_rate_check` (160, called from 59 RPC sites across 160-165) opens with an unguarded opportunistic-prune DELETE on `community_rate_events`; confirmed live in production via Sentry (VOLYUME-37, Postgres 25006 "cannot execute DELETE in a read-only transaction", scope `Community.gyms_search`, first seen 2026-09-06, dozens/minute, escalating) and corroborated in `postgres_logs`. Some connections on the app's API path are read-only (the primary database itself verified read-write via direct connection, so this is a pooler/routing condition outside this migration); every RPC sharing the rate limiter (gyms_search, gyms_near, community_find_people, community_connect, community_send_message, and more) fails outright when it lands on one. Re-issues `_community_rate_check` with the prune wrapped in its own exception handler so a prune failure can never block the real rate-check/insert; no signature, table or grant change. Rollback: re-apply the migrate_160 body (not recommended, restores the incident). | **WRITTEN, NOT APPLIED - awaiting the founder's exact phrase (urgent: live production incident, see TASKBOARD).** |
