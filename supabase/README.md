...
- **155 BLOCKED on a client fix (found 2026-09-04 during the batch
  audit; APPLIED 2026-09-26, record above).** The new INSERT policy requires `sent_on` to equal the
...
- **160 APPLIED 2026-09-07 (Community).** (Heading corrected 2026-09-11 under hostile review OJ-REV-SQL-2 F4: the table row and the 170 apply record, which verified the live objects, were already right; the body below is the pre-apply description.)
...
- **161 APPLIED 2026-09-07 (Community connections and messaging).** (Heading corrected 2026-09-11, as 160.)
...
- **162 APPLIED 2026-09-07 (UK gym directory).** (Heading corrected 2026-09-11, as 160.)
...
- **163 APPLIED 2026-09-07 (Community place + finder).** (Heading corrected 2026-09-11, as 160.)
  `migrate_163_community_place_and_finder.sql` (community product audit
  2026-09-07, `docs/community-product-audit-2026-09-07/
  30-IMPLEMENTATION.md` section 1.1, `20-JUDGEMENT.md` section 7). Five
  additive `community_profiles` columns (`place_key`, `place_label`,
  `place_lat`, `place_lng`, `place_kind`, capped to `('outward','town')`)
  plus an index. Two new SECURITY DEFINER RPCs granted to `authenticated`
  only: `gyms_place_centroid` (resolves a postcode or town to its public
  centroid without searching) and `community_set_place` (stores a chosen
  place, always at outward/town coarseness, never a device coordinate).
  `gyms_search` and `community_find_people` are DROPped and re-created
  with a new trailing parameter each (`_radius_m`, `_filters`) -- a
  different argument count is a different signature to Postgres, so the
  DROP-by-name pattern already used in `migrate_114_food_entry_weight_
  state.sql`/`migrate_116_food_library_pull_micros.sql` is reused rather
  than risking a stale duplicate overload. `gyms_search` also gains a
  server-resolved postcode centroid + radius union, an
  `operator_unconfirmed` field/tie-break, region/local-authority/country
  fields, and (`09-gym-journey-tests.md` Finding S1) a leftover-token
  fuzzy sort key so a brand+town typo is not silently excluded by the
  40-candidate cap. `gyms_near` gains the same `operator_unconfirmed`
  tie-break, an 80,468 m (50 mile) radius clamp and a `truncated` flag.
  `gyms_get`/`gyms_in_place` gain region/local-authority/country.
  `gyms_submit` is re-issued with a stripped-Jaccard duplicate check
  (Finding D2, measured 53.0%/94.7% miss rates on the real catalogue for
  the two thresholds) that strips the candidate's own brand-alias words,
  the outward code and the literal "uk" from both sides before comparing,
  closing the tokenizer asymmetry between the pipeline's `tokens` column
  and a fresh submission's tokens. `community_set_gyms` and
  `community_upsert_profile` are re-issued to backfill a missing place
  from the caller's main gym's town (never overwriting an explicit
  choice). `_community_profile_card` is re-issued (adds `place_label`,
  `age_band`, `can_connect`). `community_find_people` gains combinable
  hard filters (scope, place band, partner-only, days, time bands,
  styles, goal, experience band, age band), a keyset cursor over
  (score, last_active_at, user_id), a 1,000-row scan cap with
  `count`/`count_truncated`, muted/connect-from exclusions and the SD-28
  fallback for a thin first page. `community_suggested_people` gains the
  same muted exclusion. `community_report` accepts
  `target_kind = 'message'` (the sender is the owner; either party to the
  conversation may report; a closed conversation's messages stay
  reportable). `gyms_submit`'s duplicate check also gains the GD-06 brand
  signal (`_gyms_name_brand_id`): the submitted name's own brand, matched
  against a candidate sharing that brand AND the same postcode unit, or
  that brand AND the same sector within 150 m of a candidate's REAL
  (non-sector-fallback) coordinate, is a merge regardless of the stripped-
  Jaccard score (GD-06's own "brand 3 + postcode-unit 2 = 5, merge >= 5").
  `community_moderation_queue` is re-issued (previously untouched since
  160) to add a `'message'` content-preview branch -- the first 200
  characters of the reported message plus its conversation id, so a
  moderator sees what was actually reported instead of falling into the
  profile-lookup ELSE branch with a message id. `delete_user_data()`
  needs no re-issue -- every new column lives on `community_profiles`,
  already deleted whole-row, and no new table is created. DEPENDS ON 160,
  161 and 162. Rollback: drop the five columns and the new/re-issued
  functions this file names (see the file header), then re-apply 160
  (`community_suggested_people`/`community_report`/`community_moderation_
  queue`, never touched by 161 or 162) and 162 (everything else this file
  re-issues) to restore their pre-163 function bodies (CREATE OR REPLACE
  means the LATEST definition wins until a later file replaces it again
  -- there is no automatic revert). Not yet
  ...
  check at the end of the file (the 5 columns; the function privilege
...
- **164 APPLIED 2026-09-07 (Community final gap closure).** (Heading corrected 2026-09-11, as 160.)
...
- **165 APPLIED 2026-09-07 (Community boards, consistency counters and
  groups).** (Heading corrected 2026-09-11, as 160.) `migrate_165_community_boards_groups.sql`
...
- **155 APPLIED 2026-09-26 (founder: "run against production: 155", after confirming the build without Partners has long been in users' hands; Claude-run through the Supabase connector under the checksum protocol).** One chunk, `eea54781f8821d5580419262f1a7b90e` / 1,350 bytes, verified in the scratch table and again inside the executing DO block; the file's own `BEGIN;` and `COMMIT;` lines were removed inside that block before EXECUTE (a DO block is already one transaction and cannot run transaction commands); tracking row inserted; scratch table dropped. Verified read-only: policy "Sender writes own cheers" on `partner_cheers` is INSERT for authenticated with `sent_on = ((now() AT TIME ZONE 'UTC'))::date`, the sender check and the active-partnership check. Only 049 remains HELD.
- **186 APPLIED 2026-09-26 18:44:15 UTC (founder: "Run all against production", 2026-09-26; Claude-run through the Supabase connector under the checksum protocol, project `sujrylzzxcqxxfygptns`).** Four chunks (5,971 / 5,986 / 5,963 / 2,823 bytes) inserted into the scratch table `public.claude_apply_scratch` (RLS on, every privilege revoked from PUBLIC, anon and authenticated) as `convert_from(decode(<base64>), 'UTF8')`, each md5 and length read back and matched; the whole file `3557de002033efb8add321bbe3db1267` / 20,743 bytes re-checked inside the executing DO block before EXECUTE; the file's acceptance block passed; tracking row inserted; scratch table dropped. Verified read-only: the new-profile `share_sessions` fallback is off (`v_share_sessions := false;` in the live body), `community_upsert_profile(jsonb, boolean)` executable by authenticated and not by anon. "All" meant 186: 155 still waits for a build without Partners in users' hands (the founder is told to say when it is live), and 049 stays HELD (it drops a live table).
- **184 AND 185 APPLIED 2026-09-26, 13:49 and 13:50 UTC (founder phrase "run against production" given 2026-09-26, with "Make the decisions that bring the absolute best product for end users"; Claude-run through the Supabase connector under the checksum protocol, project `sujrylzzxcqxxfygptns`).** Order run: 184, then 185, as both files require. Method as the 176 to 183 batch: each file split locally at line boundaries into chunks of at most 6,000 bytes with a manifest of per-chunk md5s; each chunk inserted into a scratch table `public.claude_apply_scratch` (RLS on, every privilege revoked from PUBLIC, anon and authenticated; created for this batch and DROPPED at the end, 13:51 UTC) as a dollar-quoted literal, the long box-drawing rule lines rebuilt with `repeat()` so no count could drift, and its md5 and byte length read back and compared to the manifest before going on (every chunk matched first time); then one `DO` block re-assembled the chunks in order, compared the whole file's md5 and byte length to the manifest inside the block and only then executed it; then the tracking row in `claude_schema_migrations` (columns `filename`, `applied_at`); then read-only verification. Both acceptance blocks passed.
  - **Pre-apply read (read-only, 13:45 UTC):** `share_sessions` default `false`, `sessions_audience` default `'followers'`; two profiles, exactly the two the 185 header records (`alland` off/followers, `allan` on/followers, both adults by the stored flag and a fresh `_community_minor`); neither 184 change live; no scratch table present.
  - **184** 13:49:56 UTC, five chunks, whole file `d8389c40349001b8c304c8ca843550fd` / 27,476 bytes. Verified: `share_sessions` default `true` and `sessions_audience` still `'followers'` (the narrow backstop, on purpose); `community_upsert_profile(jsonb, boolean)` the only overload, carrying the ON fallback, the minor-aware audience fallback and the kept minor refusal of `'everyone'`; `community_get_me()` returns `share_sessions` and `sessions_audience`; both SECURITY DEFINER, VOLATILE, `search_path=public, pg_temp`, executable by authenticated and not by anon.
  - **185** 13:50:46 UTC, one chunk, whole file `49898f45b38fbd2c945e23e558056801` / 5,723 bytes. Verified: `alland` and `allan` both on/everyone, neither a minor; no pre-default profile left off; no minor wider than followers.
  - **Sweep after the batch:** 175 functions named `community_*`, `_community_*`, `gyms_*` or `ed_flag_push` in `public` (the same count as after 183); none missing SECURITY DEFINER or the pinned `search_path=public, pg_temp`; none executable by anon. `claude_schema_migrations` carries rows 184 and 185.
- **176 TO 183 APPLIED 2026-09-24, 14:04 to 15:33 UTC (founder phrase "run against production: 176, 177, 178, 179, 180, 181, 182, 183" given 2026-09-24, with "for all questions, take the action that brings the best product"; Claude-run through the Supabase connector under the checksum protocol, project `sujrylzzxcqxxfygptns`).** Order run: 179, 176, 177, 178, 180, 181, 182, 183 (176 before 180, as Part 0 of both files requires; 179 first and alone). Method, per file: the file split into 6,000-byte chunks locally with a manifest of per-chunk md5s; each chunk inserted into a scratch table `public.claude_apply_scratch` (RLS on, every privilege revoked from PUBLIC, anon and authenticated; created for this batch and DROPPED at the end, 15:35 UTC) as `convert_from(decode(<base64>), 'UTF8')`, its md5 and byte length read back and compared to the manifest before going on; then one `DO` block re-assembled the chunks in order, compared the whole file's md5 and byte length to the manifest inside the block and only then executed it; then the tracking row in `claude_schema_migrations`; then read-only verification. Every acceptance block (each file ends in one that raises on failure) passed.
  - **179** 14:04:25 UTC, one chunk `e5c5bceaa83268594870af742a1f1a94` / 3,607 bytes. Verified: RLS enabled on `claude_schema_migrations` and `claude_seed_files`, no anon or authenticated grants on either.
  - **176** 14:14:14 UTC, four chunks, whole file `d3310ad2e14c2888f6adc118b07fd75a` / 20,326 bytes. Verified: the three marked changes live in the three functions, each executable by authenticated and not by anon.
  - **177** 14:33:10 UTC, three chunks, whole file `0399fd9f69f4e4a146ad2f335c17993a` / 16,577 bytes. Verified: `community_respect_all` returns the `recipients` key and `community_group_join` the `admin_ids` key.
  - **178** 14:33:13 UTC, six chunks, whole file `1d88be8baec6219a9ff10397d6abdce9` / 30,396 bytes. Verified: `community_posts_kind_check` carries `'note'`, `_community_payload_keys` has the note branch, `community_create_post` calls `_community_payload_has_blocked_term`, which no client role can execute.
  - **180** 15:13:22 UTC, fifteen chunks, whole file `91cf47e9d27eea4a19ed414ae6a83d7a` / 84,502 bytes. Part 0 passed because 176 was already live. Verified: `_community_ed_flag_open`, `_community_calm_mode_on` and `_community_consistency_withheld` present, STABLE, SECURITY DEFINER on the pinned search_path, executable by no client role; the live bodies of all six readers call `_community_consistency_withheld`; `community_update_training_profile` calls `_community_ed_flag_open(v_uid)`; the five client RPCs executable by authenticated and not by anon; open ED-pattern flags in the cloud table at apply: 0.
  - **181** 15:23:26 UTC, three chunks, whole file `9ac736c2e337acbdc88dfd08541e29c6` / 13,982 bytes. Chunk 2 was first inserted with one extra box-drawing character in a rule line (a transcription miscount: 6,003 bytes for 6,000), caught by the per-chunk md5 check, corrected in place with one targeted UPDATE and re-verified against the manifest before the whole-file check; nothing executed until every chunk matched. Verified: `gyms_pending_submissions` and `gyms_pending_reports` present, STABLE, SECURITY DEFINER on the pinned search_path, gated on `community_is_moderator()`, executable by authenticated and not by anon.
  - **182** 15:30:13 UTC, three chunks, whole file `ebc724923b8155be65c84363024d4523` / 16,177 bytes. Verified: `ed_flag_push(uuid, timestamptz, timestamptz, text)` present, VOLATILE, SECURITY DEFINER on the pinned search_path, executable by authenticated and not by anon, its body scoped `WHERE user_id = v_uid AND id = _id`, `FOR UPDATE`, `ON CONFLICT (user_id, id) DO NOTHING`, no signals parameter; the only policy left on `ed_pattern_flags` is "Users can read own ed_pattern_flags"; anon and authenticated cannot INSERT, UPDATE or DELETE the table and authenticated can still SELECT it (the pull); open flags at apply: 0. The ED arm of 180 and both edge functions' ED gates go live the moment a device on a build carrying the push raises a flag; no such build has shipped yet.
  - **183** 15:33:13 UTC, two chunks, whole file `2b51f2c589f75a856fa6c69e7d27eccf` / 7,657 bytes. Verified: `community_dimensions_me(text)`, `gyms_in_place(text, integer)` and `community_gym_suggest(text, text)` executable by neither anon nor authenticated; `gyms_near(double precision, double precision, integer, integer)` still executable by authenticated.
  - **Sweep after the batch:** 175 functions named `community_*`, `_community_*`, `gyms_*` or `ed_flag_push` in `public`; none missing SECURITY DEFINER or the pinned `search_path=public, pg_temp`; none executable by anon. `claude_schema_migrations` carries rows 176 to 183.
  - **Edge function `community-notify` redeployed as version 4, 2026-09-24 15:38 UTC** (`verify_jwt` true, entrypoint `functions/community-notify/index.ts` plus `functions/_shared/boundedJson.ts`, through the connector's deploy tool). Version 3 (2026-09-11) lacked two landed changes: the note-Respect push copy (item 5, F10, `55937478`) and the `group_accepted` proof's `joined_at` recency bound (item 9, R-02, `c10b85f0`). The live source was fetched back after the deploy and compared to the repo files (md5 `94a4f00574f4add915b7d9ea76108e6c` / 41,693 bytes and `ea41797722cdef271342818c0e0d7ba2` / 1,764 bytes): IDENTICAL, both md5s matched and a line diff was clean (the transcription of the fetched source was made from the connector's returned copy, not from the repo, so the check is independent).
- **172 APPLIED 2026-09-11 13:00 UTC (Community: `community_profiles.c_prs_4w`, the "3 PRs in 4 weeks" strip figure; same batch and phrase as 171; two chunks verified md5 `5e03402c31b63a88888bb7b3051dd530` / 10,738 and `e8ab0b83ee1ae9156c5656bbf6077ef6` / 11,202 bytes, whole file `3e178b883448cd2f43f86b8ad09e0571` / 21,940 re-checked inside the executing DO block; acceptance block passed; verified read-only: the column present, both re-issued functions carry `c_prs_4w`, the training-profile RPC VOLATILE and executable by authenticated, the card helper executable by no client role).** Written 2026-09-11 from the fresh-eyes review of phases 0 to 3 (finding F8: blueprint section 4 required the PR count on the progress strip when the person shares what they did, and neither the schema nor the client ever built it). One additive smallint column; `community_update_training_profile` and `_community_profile_card` RE-ISSUED IN FULL, carried forward byte-for-byte from migrate_165 (lines 298-517) and migrate_170 (lines 765-866) with only the marked lines added, proved by `src/__tests__/migrate172.rpcOnly.guard.test.js`; the count is accepted under the counters' share_consistency gate (clamped 0..200, minors and non-active callers never) and exposed only when the owner shares consistency AND shares what they did. A count, never a lift. Client half (device counter, publish, strip cell) lands after the apply. Rollback: see the file header.
- **177 APPLIED 2026-09-24 14:33 UTC (written 2026-09-22; founder order 2026-09-22, item 1, "wire the pushes": two Community actions could not notify because the server never told the client who to notify -- STOP comments recorded in `src/lib/community/respect.js` and above `joinGroup` in `src/lib/community/groups.js`). Applied under the founder's phrase of 2026-09-24, Claude-run through the connector under the checksum protocol: three chunks verified, whole file `0399fd9f69f4e4a146ad2f335c17993a` / 16,577 bytes re-checked inside the executing DO block; verified read-only: `community_respect_all` returns the `recipients` key and `community_group_join` the `admin_ids` key (apply record below).** Two functions re-issued byte-for-byte with the marked changes only (guard-proved by `src/__tests__/migrate177.guard.test.js`): `community_respect_all(text, text, text)` from migrate_170 lines 3744-3888 now returns `{given, recipients: [{user_id, post_id}]}`, exactly the recipients it gave a brand-new reaction to on this call (never an already-respected one, never the caller); `community_group_join(uuid)` from migrate_165 lines 933-979 now returns `{state, admin_ids}` on the 'requested' branch only (the group's current admins, the same set the pre-existing PERFORM already notifies in-app), `{state}` alone on 'member'. No table changes, no signature or grant changes. Client half (same landing): `respectAll`/`joinGroup` notify `reaction`/`group_request` for each recipient/admin id, tolerating an older server that omits the new keys. Rollback: re-issue both from 170 and 165 (file header).
- **178 APPLIED 2026-09-24 14:33 UTC (written 2026-09-22; Community: a first post without a
  workout, founder order 2026-09-22 item 5; audit evidence
  `docs/audit/community-audit-2026-09-22/A-adoption-visibility-look-copy.md`
  finding A-05: every post kind is generated from logged training data, so
  a curious new joiner with no session logged yet could post nothing).
  Applied under the founder's phrase of 2026-09-24, Claude-run through the
  connector under the checksum protocol: six chunks verified, whole file
  `1d88be8baec6219a9ff10397d6abdce9` / 30,396 bytes re-checked inside the
  executing DO block; verified read-only: the kind CHECK carries 'note',
  `_community_payload_keys` has the note branch, `community_create_post`
  calls the blocked-term helper, which no client role can execute (apply
  record below).** Adds a
  short free-text post kind, `'note'`: no payload keys at all, the existing
  `caption` field (cleaned and capped at 280, same as every other kind) is
  the whole post. `community_posts_kind_check` widened to add `'note'`,
  keeping `'pr', 'session', 'block', 'milestone', 'programme'`.
  `_community_payload_keys(text)` (migrate_160 lines 652-675) re-issued
  byte-for-byte with one marked addition: `'note'` returns an empty
  `text[]`, never NULL (NULL is what the create path reads as an unknown
  kind). `community_create_post` (last defined migrate_170 lines
  2981-3185) re-issued byte-for-byte with one marked addition: a `'note'`
  post must carry a non-empty caption, checked AFTER cleaning so a
  blocked-term caption still raises `content_not_allowed` first, never
  masked as `invalid_input`. Confirmed unchanged, so NOT touched by this
  file: `_community_clean_text` already runs on every kind's caption
  unconditionally (migrate_170 line 3027), so the blocked-terms check
  already covers a note; `community_feed`, `community_discover_posts`,
  `community_dimension_recent` and `community_group_feed` filter on
  visibility, author status, block/mute and group membership only, never
  on `kind`, so a visible note reaches every list a visible post of any
  other kind reaches; `_community_post_json` (migrate_160, never
  re-issued) names `kind`/`payload`/`caption` generically for every kind.
  No table is created or dropped. Guard: `src/__tests__/migrate178.guard.
  test.js`. Rollback: drop and re-add `community_posts_kind_check` without
  `'note'`; re-issue `_community_payload_keys` from migrate_160 lines
  652-675 and `community_create_post` from migrate_170 lines 2981-3185
  (file header).
- **176 APPLIED 2026-09-24 14:14 UTC (written 2026-09-13; Community early days, CR-16 / D162; found by the two-account production proof of 2026-09-13: the admin's Hub still listed "Volt Gym crew" after `community_group_close`). Applied under the founder's phrase of 2026-09-24, BEFORE 180 as both files require, Claude-run through the connector under the checksum protocol: four chunks verified, whole file `d3310ad2e14c2888f6adc118b07fd75a` / 20,326 bytes re-checked inside the executing DO block; verified read-only: all three marked changes live, the three RPCs executable by authenticated and not by anon (apply record below).** Three functions re-issued byte-for-byte with one marked change each (guard-proved by `src/__tests__/migrate176.rpcOnly.guard.test.js`): `community_hub_summary(text)` from migrate_170 lines 1550-1705 keeps `g.status = 'active'` (a closed group leaves the Hub's GROUPS section); `community_group_list_mine()` from migrate_165 lines 1212-1233 keeps active groups only (a closed group leaves "My groups" and the Join screen's groups check); `community_group_accept_invite(uuid, uuid)` from migrate_165 lines 1146-1208 re-reads the group after the member count is incremented, so the card it returns carries the count after the join (the client reloads the page anyway, so nothing rendered wrong). No table changes. Rollback: re-issue the three from 170 and 165 (file header).
- **175 APPLIED 2026-09-12 15:05 UTC (Community: the rules gate tolerates an older client, D160; founder order 2026-09-12 "just finish everything so it is ready when I build", completing the batch the phrase of 2026-09-11 authorised; hostile-reviewed by Opus, OJ-REV-SQL-3: APPLY, its two hardening notes taken; Claude-run through the connector under the checksum protocol: two chunks verified md5 `6255e96d70bee59436d97d293cb9bcd3` / 12,864 and `1e173672e927a734b8bfde4132f0e36c` / 12,372 bytes, whole file `334c3fe7a8fcd6623f1fa4c1b9eb0f74` / 25,236 re-checked inside the executing DO block; acceptance block passed; verified read-only: rules version 3 live again, the two-argument signature only, VOLATILE, executable by authenticated and not by anon, the tolerant gate and the published-notice bound each present exactly twice in the live body and the exact-equality gate absent, the act-level gate intact, all community functions SECURITY DEFINER on the pinned search_path and none anon-executable; `claude_schema_migrations` row inserted and the scratch table dropped).** `community_upsert_profile(jsonb, boolean)` re-issued byte-for-byte from migrate_170 lines 2597-2902 with four marked changes (guard-proved): the two exact-equality `accept_rules_version` gates become the range 1 to the server's version plus one, the version RECORDED is bounded to the one the server has published (hostile review OJ-REV-SQL-3 F5: a consent row must be evidence of a notice that existed; a build one version ahead is accepted, recorded at the current version, and re-accepts once when its migration lands), and the create path stores that version in the profile row and the consent log. An old build keeps its profile, hub, gyms and boards and is asked for the current rules by `_community_require_rules` (unchanged) on connect, message and training-profile share; a build one version ahead of a not-yet-applied migration is accepted rather than refused on every write. Part 2 restores `_community_rules_version()` to 3 in the same transaction (the 174 part 2 stopgap rollback of 14:33 UTC is undone by this file). Client half (same landing): the rules screen shows an "update Volyume" line instead of an accept button when the server's version is ahead of the text the build carries (`rulesTextBehindServer`). Guard: `src/__tests__/migrate175.rpcOnly.guard.test.js`. Rollback: re-issue the upsert from 170 and the version from 174 or 161 (file header).
- **173 APPLIED 2026-09-11 22:34 UTC (Community at onboarding, CR-15 / D158 / D159: `community_handle_suggestion(_hint)`, the server-side handle suggestion the onboarding step and the Join screen pre-fill; founder phrase "run against production" given 2026-09-11 for the 173 + 174 batch; Claude-run through the connector under the checksum protocol, SECOND in the batch after 174: two chunks verified md5 `3e2ef5b4168934df44b819b7f604a757` / 11,107 and `0c979c6500729e35826d0b05c948aaa5` / 9,241 bytes, whole file `7010e656ae202a9b27e710c06772a636` / 20,348 re-checked inside the executing DO block; acceptance block passed; verified read-only: the one `(text)` signature and no zero-argument overload, VOLATILE, executable by authenticated and not by anon, both helpers IMMUTABLE and executable by no client role, the sanitiser cases right, all 157 `community_*`/`_community_*` functions SECURITY DEFINER on the pinned search_path and none anon-executable; `claude_schema_migrations` rows for 174 and 173 inserted 22:34:25 UTC and the scratch table dropped).** Written 2026-09-11 from `docs/communities-revamp-2026-09-10/25-ONBOARDING-COMMUNITY-SPEC.md` section 4.1; amended the same day under D159 Q2 (the founder delegated the data-minimisation question on the "best app" criterion): the person's own name first (the `_hint` the onboarding step passes, the name typed at step 2; then `users_profile.first_name`; then the sign-in provider's given name), the sign-in address only as a last resort and only its LEADING LETTERS up to the first separator or digit (never the full local part), then `athlete`; collisions `_2`..`_99` then four random digits, at most 120 tries. Three functions, no table changes: `_community_handle_suggest_reserved()` (the client's 80 reserved words, byte-for-byte, no client grant), `_community_handle_base(text)` (the pure sanitiser, no client grant), and the RPC (VOLATILE for its rate rail, 30 an hour; an existing member gets their own handle back before the rail; the sign-in record is read only when the name sources gave nothing and the address never leaves the function). Runs AFTER 174 in the batch (F1 above). `_community_handle_reserved()` is deliberately NOT widened (the upsert re-validates the merged handle on every save). Hostile-reviewed (Opus) twice: the first review's four FIX FIRST items landed; the amended derivation reviewed before apply. Guard: `src/__tests__/migrate173.rpcOnly.guard.test.js`. Rollback: three DROP FUNCTIONs (file header).
- **174 APPLIED 2026-09-11 22:34 UTC; PART 2 ROLLED BACK TO `SELECT 2` AS A STOPGAP 2026-09-12 14:33 UTC (the Community builds on the founder's own devices, iOS 2.0.0+65 and Android 3573 to 3575, send rules version 2; the store apps carry Community only from the next published build and the upsert's gate is exact equality, so every profile create and edit from them was refused while the server said 3; no profile write reached the server in the window; the durable fix is migration 175, which makes the gate tolerant of an older client and restores 3 in the same transaction, APPLIED 15:05 UTC the same day, version 3 live again; founder order 2026-09-12 "just finish everything so it is ready when I build ... decisions ... make them in what brings the best product") (Community, D159 Q3 and Q4, same batch and phrase as 173; ran FIRST in the batch, before 173, one chunk verified md5 `0e2c53a320838b910941c9b9ed17b384` / 9,129 bytes re-checked inside the executing DO block, acceptance block passed, verified read-only: rules version 3, an unknown account reads as a minor, `_community_minor` STABLE and `_community_rules_version` IMMUTABLE on the pinned search_path, no existing member flagged (one member, with a cloud date of birth); the order mattered: the client on main sends rules version 3 on every profile write and `community_upsert_profile` requires exact equality, so until 174 is applied every Community profile create and edit fails `invalid_input`, 173's onboarding join included; hostile review OJ-REV-SQL-2 F1).** Part 1: `_community_minor(uuid)` re-issued from migrate_160 with its three fail-open returns turned to TRUE, so an absent, blank or unparseable date of birth reads as a minor until the date arrives (followers-only, out of discovery, cohorts, boards and groups); recomputed on every `community_upsert_profile`; verified before writing through a read-only count that every existing member has a cloud date of birth, so nobody changes state on apply; the client half pushes the body profile row before every profile write and forces it before the onboarding join (`pushUserBodyProfileNow`, `ensureBodyProfilePushed`). Part 2: `_community_rules_version()` returns 3, matching the client's `COMMUNITY_RULES_VERSION` and the version-3 rules text, so a member who accepted version 2 re-accepts once through the existing `rules_outdated` path. Guard: `src/__tests__/migrate174.rpcOnly.guard.test.js`. Rollback: re-issue both functions from 160 and 161 (file header).
- **171 APPLIED 2026-09-11 13:00 UTC (Community: `community_friends_trained_today(_today)`, the count-only RPC for the home-screen widget's "N friends trained today" line; founder phrase "run against production" given 2026-09-11; Claude-run through the Supabase connector under the checksum protocol: one chunk verified md5 `f114ac17c2885e67602fd490347a6958` / 7,997 bytes, executed inside a DO block that re-checks the checksum before EXECUTE; acceptance block passed; verified read-only: present, STABLE, SECURITY DEFINER with the pinned search_path, executable by authenticated and not by anon; every community function still pinned, every community table still RLS-on with no client grant). The client switched to it the same day.** Written 2026-09-11 from the phase 4 fresh-eyes review (finding 9): the client derived one integer by loading up to fifty profile cards through `community_board`'s `following` scope; this RPC returns the integer and nothing else, computed with exactly the board's `following` eligibility (active, adult, sharing consistency, counters inside fourteen days, not blocked either way, an accepted follow, `c_last_trained_day = _today`), the caller's own row excluded, capped at 999. STABLE (no rate-rail write, no DML), SECURITY DEFINER on the pinned search_path, EXECUTE to authenticated only; `_today` follows 170's contract. Guard: `src/__tests__/migrate171.rpcOnly.guard.test.js`. The client keeps its board path until this is live, then switches (one landing). Rollback: DROP FUNCTION.
- **170 APPLIED 2026-09-10 (Community connection, parts A, A2 and B in one file; founder phrase "Run against production" given 17:58 UTC; applied Claude-run through the Supabase MCP connection, project `sujrylzzxcqxxfygptns`).** `apply-named-sql.yml` run #11 (main `ec4d241`) had FAILED at 17:59:18Z with HTTP 401 Unauthorized from the Management API before any statement (the repo secret `SUPABASE_ACCESS_TOKEN` is no longer accepted; runs #5 to #10 on 2026-09-07 used it). The founder then said "You have the connector", and the lead applied the file through the MCP `execute_sql` path under a checksum protocol: a scratch table `public.claude_apply_scratch(seq, part, body)` (RLS on, all privileges revoked from PUBLIC, anon and authenticated) received the file in twelve dollar-quoted chunks (six for part A, one for A2, five for B); every chunk was verified by `md5(body)` and `octet_length(body)` against the local file (`sed -n 'a,bp' | md5sum`) and four chunks that stored one byte short at an empty boundary line were corrected and re-verified; each part's concatenation was verified against the whole-part checksum (A2 `cd0391dde5641eab8bee3cbd1eebeb9c` / 11,913 bytes; B `c5112e417f90ef703946aae38373ec74` / 91,829 bytes) and then executed atomically in one `DO $$ BEGIN EXECUTE (SELECT string_agg(body, '' ORDER BY seq) ...) END $$` statement, so a failure anywhere in a part rolls the whole part back. Part A executed first (its acceptance block passed), then A2, then B (its acceptance block passed) at about 19:04 UTC; `claude_schema_migrations` row inserted 19:05:22 UTC; the scratch table was dropped 19:06:16 UTC. Verified read-only afterwards, all true: `community_dimension_recent` (one overload); `community_respect_all(text, text, text)` and `community_post_set_note(uuid, text)` present and executable by authenticated; `community_upsert_profile(jsonb, boolean)` and the eight-argument `community_create_post` present with the old one-argument and five-argument overloads GONE; `community_post_groups` and `community_notify_daily` present. Sweep over every `community_*` / `_community_*` function (154) and `delete_user_data`: all SECURITY DEFINER with a pinned `search_path`; none executable by anon; the migrate_170 writers (`community_upsert_profile`, `community_create_post`, `community_post_set_note`, `community_respect_all`, `community_hub_summary`, `community_dimension_recent`, `community_dimensions_me`) are VOLATILE (migrate_167's lesson), `_community_can_view_post` STABLE and `_community_connect_reasons_list` IMMUTABLE; 22 `community_*` tables, every one with RLS on and none granted to anon or authenticated. Two PRE-EXISTING observations, not changed by 170 (no statement in 170 names them): `_community_rate_check(uuid, text, integer, integer, interval)` is executable by authenticated (granted explicitly by migrate_166 and re-issued by migrate_167); the nine `community_*programme*` RPCs are not executable by authenticated (the retired programme-sharing surface). `supabase/functions/community-notify` redeployed as version 3 through the MCP `deploy_edge_function` path at 2026-09-11 05:19 UTC with `verify_jwt` true (as before), entrypoint `functions/community-notify/index.ts` plus `functions/_shared/boundedJson.ts`; the deployed source was fetched back and diffed against the repo file: identical (md5 `9405c1a0260e37563a6c6ea244246b59`). This function now collapses `reaction` pushes to one per recipient per UK-local day through `community_notify_daily`. FOUNDER RULING 2026-09-11 (standing): the repo's Supabase token is NOT rotated and is never asked for again. Cloud migrations and edge-function deploys run through the Claude session's Supabase connector (MCP), under the same production phrase and the checksum protocol recorded in the `supabase/README.md` 170 status block; `apply-named-sql.yml` and `deploy-functions.yml` are not the route.
  What 170 carries (communities revamp 2026-09-10; `docs/communities-revamp-2026-09-10/20-BLUEPRINT.md` sections 3, 4, 7, 8; `21-PHASE1-SPEC.md` section 5; `23-PHASE3-SPEC.md`; contract at `22-MIGRATION-170A-CONTRACT.md`). Part A: `community_profiles.discipline_keys` (15-key taxonomy, max 3) plus its validating/labelling helpers; `_community_profile_card` gains discipline fields and the nine migrate_165 consistency counters (gated on the owner's `share_consistency`/`status`/`is_minor`); `community_dimension` and `community_dimensions_me` (DROPped and re-created with a new `_today` param, NULL falling back to the UK-local day) gain `discipline`/`age_band` cohorts with member/trained-today counts and a sample, via new helper `_community_cohort_stats`; `community_board` gains scopes `area`/`style`/`discipline`/`age_band`; new `community_hub_summary(_today)` returns cohorts and groups in one call; `community_find_people` (DROPped and re-created) gains a hard `_discipline` filter. Part A2: `community_dimension_recent` (a cohort's recent stories) and group count alignment. Part B: `share_sessions` / `sessions_audience` / `c_planned_per_week` on `community_profiles`, `auto` / `client_ref` on `community_posts` with the `(author, client_ref)` unique index, the `'groups'` visibility with the new RPC-only table `community_post_groups`, `community_post_set_note`, `community_respect_all` (rate-railed, blocked and muted pairs excluded server-side), Together figures on `community_group_get`, the daily digest table `community_notify_daily`, `same_discipline` replacing `same_programme` in the connect reasons, and `delete_user_data()` re-issued naming both new tables. DEPENDS ON 160-165, 167, 169. Rollback: see the file header.
...
| 160 | `migrate_160_community.sql` | Community (Social / Community / Discovery campaign, `docs/social-discovery-2026-09-06/30-BLUEPRINT.md` section 3). Fourteen `community_*` tables (profiles, follows, blocks, mutes, programmes + uses, posts, reactions, comments, reports, moderators, moderation log, activity, rate events), all RLS-enabled with NO anon/authenticated policy and ALL privileges revoked from both; 41 SECURITY DEFINER RPCs pinned to `search_path = public, pg_temp` as the only ingress and egress (SD-14). Widens the `consent_log` CHECK (`community_visibility`) and the `notification_preferences` category CHECK (`community_follow`, `community_activity`); seeds `community_moderators`; re-issues `delete_user_data()` in full with two-sided Community deletes. Rollback: drop the fourteen tables and the `community_*` / `_community_*` functions, re-apply migrate_154, re-narrow both CHECKs (see the file header). | **APPLIED 2026-09-07** (`apply-named-sql.yml`, Management API path, batch with 160-163 or 164-165 as applicable). |
| 161 | `migrate_161_community_connections.sql` | Community connections, messaging, the shared training profile and the discovery surfaces (Discovery campaign, `docs/social-discovery-2026-09-06/70-DISCOVERY-BLUEPRINT.md` section 11; SD-20 to SD-32). Thirteen additive `community_profiles` columns (`connect_from`, `open_to_partner`, `partner_prefs`, `show_programmes`, `connection_count`, eight `tp_*` bands); three new tables (`community_connections`, `community_conversations`, `community_messages`), all RLS-enabled with NO anon/authenticated policy and ALL privileges revoked from both; 23 SECURITY DEFINER RPCs pinned to `search_path = public, pg_temp` as the only ingress and egress (SD-14). Widens the `community_activity.kind` CHECK (`connect_request`, `connect_accepted`), the `community_reports.target_kind` CHECK (`message`) and the `notification_preferences.category` CHECK (`community_message`). Moves the Community rules to version 2 with a re-consent path and the new `rules_outdated` refusal. Re-issues `_community_profile_card`, `community_get_me`, `community_upsert_profile`, `community_block`, `community_unfollow`, `community_leave` and `delete_user_data()` in full, the last with two-sided deletes for the three new tables. DEPENDS ON 160. Rollback: drop the three tables and the thirteen columns, drop the `community_*` / `_community_*` functions this file creates, re-apply migrate_160, re-narrow the three CHECKs (see the file header). | **APPLIED 2026-09-07** (`apply-named-sql.yml`, Management API path, batch with 160-163 or 164-165 as applicable). |
| 162 | `migrate_162_gym_directory.sql` | The UK gym master database (founder brief 2026-09-06, `docs/gym-database-2026-09-06/20-BLUEPRINT.md`, GD-01 to GD-17), schema only, no venue rows. Seven new tables: `gym_brands`/`gym_venues`/`gym_postcode_sectors` (`global_read_only`, RLS on, one SELECT policy for `authenticated`) and `gym_venue_sources`/`gym_venue_history`/`gym_submissions`/`gym_reports` (`rpc_only`, RLS on, no policy, all privileges revoked from anon/authenticated). Two additive `community_profiles` columns (`gym_id`, `other_gym_ids`, capped at 3) plus a trigger deriving `gym_key`/`gym_label` from `gym_id`. Eleven new SECURITY DEFINER RPCs pinned to `search_path = public, pg_temp`, granted to `authenticated` only: `gyms_search`, `gyms_near`, `gyms_in_place`, `gyms_get`, `gyms_suggest`, `gyms_submit`, `gyms_confirm_submission`, `gyms_report`, `gyms_review_submission`, `gyms_review_report`, `community_set_gyms`. Re-issues `community_gym_summary` (resolves `gym:<uuid>` keys from `gym_venues`, keeps the legacy free-text path), `community_gym_suggest` (delegates to `gyms_suggest`, same signature) and `delete_user_data()` in full (anonymises `gym_submissions.submitter_id`/`gym_reports.reporter_id`). DEPENDS ON 160 and 161. Rollback: drop the seven tables and the two columns, drop the `gyms_*`/`_gyms_*` functions and `_community_gym_key_sync`, re-apply migrate_161 to restore `community_gym_summary`/`community_gym_suggest`/`delete_user_data()` to their 161 bodies (see the file header). | **APPLIED 2026-09-07** (`apply-named-sql.yml`, Management API path, batch with 160-163 or 164-165 as applicable). |
| 163 | `migrate_163_community_place_and_finder.sql` | Community place + finder (community product audit 2026-09-07, `docs/community-product-audit-2026-09-07/30-IMPLEMENTATION.md` section 1.1). Five additive `community_profiles` columns (`place_key`, `place_label`, `place_lat`, `place_lng`, `place_kind`, capped `('outward','town')`) plus an index. Two new SECURITY DEFINER RPCs granted to `authenticated` only: `gyms_place_centroid`, `community_set_place`. `gyms_search`/`community_find_people` DROPped-and-recreated with a new trailing parameter each (`_radius_m`, `_filters`); `gyms_search` also gains a postcode centroid + radius union, `operator_unconfirmed`, region/local-authority/country and a Finding-S1 fuzzy sort key; `gyms_near` gains `operator_unconfirmed`, an 80,468 m radius clamp and `truncated`; `gyms_get`/`gyms_in_place` gain region/local-authority/country; `gyms_submit` gets a stripped-Jaccard duplicate check (Finding D2) plus the GD-06 brand signal (same brand + same postcode unit, or same brand + same sector within 150 m of a real coordinate, is a merge regardless of Jaccard). `community_set_gyms`/`community_upsert_profile` re-issued to backfill place from the main gym's town; `_community_profile_card` re-issued (`place_label`, `age_band`, `can_connect`); `community_find_people` gains combinable `_filters`, keyset paging, a 1,000-row scan cap and the SD-28 fallback; `community_suggested_people` re-issued for the mute exclusion; `community_report` accepts `target_kind = 'message'`; `community_moderation_queue` re-issued with a message content preview (first 200 characters + conversation id). DEPENDS ON 160, 161 and 162. Rollback: drop the five columns and the new/re-issued functions, re-apply 160 (community_suggested_people/community_report/community_moderation_queue) and 162 (everything else) to restore their pre-163 bodies (see the file header). | **APPLIED 2026-09-07** (`apply-named-sql.yml`, Management API path, batch with 160-163 or 164-165 as applicable). |
| 164 | `migrate_164_community_gap_closure.sql` | Final competitive gap closure (community product audit 2026-09-07, `docs/community-product-audit-2026-09-07/40-GAP-CLOSURE.md`). Retires the shared-programme layer's nine RPCs (EXECUTE revoked from everyone; tables stay, hold no rows); re-issues `community_find_people`/`community_suggested_people`/`community_dimension`/`community_comment`/`community_report` without the programme signal. Two additive `community_profiles` columns (`show_gym`, `show_place`) plus their setters, enforced in `_community_profile_card` and `community_find_people`. `community_search_people` re-issued (substring name match, 40-candidate cap). `community_list_followers` and `community_my_status` added. `community_messages` gains `ref_payload`; `community_send_message` DROPped-and-recreated with a session-suggestion payload; `community_respond_session` added. `notification_preferences` gains a quiet-hours sentinel row (`quiet_start`/`quiet_end`/`tz`) plus `community_set_quiet_hours`. DEPENDS ON 160, 161, 162 and 163. Rollback: see the file header. | **APPLIED 2026-09-07** (`apply-named-sql.yml`, Management API path, batch with 160-163 or 164-165 as applicable). |
| 165 | `migrate_165_community_boards_groups.sql` | Community boards, consistency counters and groups (community product audit 2026-09-07, `docs/community-product-audit-2026-09-07/60-DESIGN-PROGRESS-COMMUNITY.md` sections 1-3 and 6, `40-GAP-CLOSURE.md` sections 4-5). Nine additive `community_profiles` columns (eight `c_*` consistency counters plus `share_consistency`); `community_update_training_profile` re-issued to accept the counters only when `share_consistency` is true. `community_board` added (four scopes, three windows, keyset paged, ranked over the whole eligible set). Three new tables (`community_groups`, `community_group_members`, `community_group_invites`), RLS on with no policy, all privileges revoked from anon/authenticated; sixteen new group RPCs plus `community_group_feed`; minors refused everywhere; last-admin-cannot-leave rule. Widens `community_activity.kind` (three group kinds, riding the `community_follow` category) and `community_reports.target_kind` (`group`); `community_report`/`community_moderation_queue` re-issued for group reports. `delete_user_data()` re-issued in full with two-sided group deletes and the promote-or-close rule. `supabase/functions/community-notify/index.ts` updated for the three group notification kinds. DEPENDS ON 160, 161, 162, 163 and 164. Rollback: see the file header. | **APPLIED 2026-09-07** (`apply-named-sql.yml`, Management API path, batch with 160-163 or 164-165 as applicable). |
| 166 | `migrate_166_community_rate_check_hotfix.sql` | **SUPERSEDED BY 167 — DIAGNOSIS WAS WRONG.** Guarded `_community_rate_check`'s opportunistic-prune DELETE with an exception handler on the theory (Sentry VOLYUME-37, Postgres 25006) that some app-side connections were landing read-only (a "pooler/routing condition"). That was an inference stated as fact, and it was incorrect: applied to production 2026-09-07, it made no measurable difference — Sentry recorded the identical error on the very next call, now on the mandatory INSERT a few lines below the guarded DELETE. See 167 for the real cause and fix. | **APPLIED 2026-09-07** (`apply-named-sql.yml`, run #7) — **functionally superseded by 167**, which reverts this migration's exception guard once the real cause is fixed. |
| 167 | `migrate_167_gyms_stable_volatility_fix.sql` | **REAL ROOT-CAUSE FIX for VOLYUME-37.** Read via `pg_get_functiondef` against the live database (not inferred): `gyms_search`, `gyms_near`, `gyms_in_place`, `gyms_get` and `gyms_place_centroid` (all re-issued by 163) are declared STABLE. PostgREST forces a hard READ ONLY transaction for any RPC call to a STABLE/IMMUTABLE function regardless of HTTP verb; all five call `_community_rate_check`, which writes (DELETE + INSERT). Every call to any of the five was therefore guaranteed to fail on the write, deterministically, not intermittently — matching both founder reports (no results within any radius with location on, and a plain name search failing with no location at all; both paths call these functions). No other Community RPC calling `_community_rate_check` is STABLE. Fix: re-issues the five functions with bodies unchanged and STABLE removed (now VOLATILE); reverts 166's exception guard on `_community_rate_check` back to a plain, unguarded DELETE, since the real cause is fixed and swallowing errors was never a cure. DEPENDS ON 160, 162, 163. Rollback: re-apply 163's versions of the five functions and 166's guarded rate-check (not recommended, restores the incident). | **APPLIED 2026-09-07** (`apply-named-sql.yml`, run #8). Verified against production: all five functions `provolatile = 'v'`, `_community_rate_check` carries no exception guard. Verified against Sentry (`search_events`, `errors` dataset, 1h window): the last "read-only transaction" event is 16:52:47Z, before this deploy (17:12:57Z) — zero new occurrences since. |
| 168 | `migrate_168_gym_finder_relevance_and_order_fix.sql` | Two pre-existing gym-finder defects, surfaced by founder device testing the moment 167 made the finder reachable again (not part of VOLYUME-37, not caused by 166/167). (1) `gyms_near` ordered `operator_unconfirmed ASC, distance_m ASC` — verification status BEFORE distance, so a confirmed venue miles away sorted ahead of a closer unconfirmed one on a screen whose entire point is "nearest first" (device report: Anytime Fitness Southport 4.2mi listed above Formby Hall Golf Resort 0.4mi). Fixed: distance is now the primary key, verification the tie-break. (2) `gyms_search`'s token-overlap match qualifies a venue on ANY shared token with no distinction between a distinguishing word and a generic venue-type word; confirmed directly against production, "volt gym" matched 4,075 of ~10,600 open venues on "gym" alone, swamping the 4 real Volt-named venues within the 40-row cap. Fixed: a fixed generic-word list (gym, fitness, club, centre, leisure, studio, health, academy, training, sports, bare stopwords) is stripped from the tokens used to qualify a match, falling back to the full set only when every typed token is generic; confirmed the fix drops "volt gym"'s candidate count to the correct 4. A third, client-only defect in the same investigation (`src/components/community/GymPicker.js`: the near-list was unconditionally merged into text-search results, so a typed name like "volt" alone still showed a page of unrelated nearby venues ahead of the real matches) is a JS fix, not a migration — see the commit for that file. DEPENDS ON 160, 162, 163, 167. Rollback: re-apply 167's versions of `gyms_search`/`gyms_near` (not recommended, restores both defects). | **APPLIED 2026-09-07** (`apply-named-sql.yml`, run #9). Verified against production: `gyms_near` carries the distance-first ORDER BY; `gyms_search` carries the generic-word list; re-ran the "volt" match-count check directly against live `gym_venues` — 4 matches (correct), down from the pre-fix 4,075 for "volt gym". |
| 169 | `migrate_169_community_board_x_alias_fix.sql` | `community_board` (165, the "My gym"/"Following"/"Everyone"/group consistency board) has failed on EVERY call since it was deployed — a pure SQL syntax defect, not caused by anything today, invisible to the JS unit suite (no local Postgres in CI for SECURITY DEFINER functions). Founder device report ("Could not load this board"); Sentry VOLYUME-37 changed error under the same grouping to Postgres 42P01 "missing FROM-clause entry for table \"x\"", first seen the day 165 shipped. Root cause read off the live function: all three ranking CTEs wrote `SELECT x.*, ... FROM (SELECT unnest(v_items) AS x) u` — `x` is the column alias from `unnest`, not a table alias (the derived table itself is `u`), so `x.*` asks Postgres to expand a table that does not exist. Every other Community function using this identical keyset-paging idiom (160-164) correctly writes `SELECT u.x`/`SELECT t.x`; this was a one-off copy-paste slip isolated to `community_board`, confirmed by grepping every `unnest(v_items) AS x` site in the repo. Fix: `SELECT x.*,` → `SELECT u.x,` in all three CTEs, verified to run correctly against a fabricated `v_items` array before writing this migration. No other change to the function. DEPENDS ON 165. Rollback: re-apply 165's version of `community_board` (not recommended, restores the 100%-failure defect). | **APPLIED 2026-09-07** (`apply-named-sql.yml`, run #10). Verified against production: the function definition no longer contains `SELECT x.*,` and now contains `SELECT u.x,`. Verified against Sentry (`search_events`, errors dataset, 1h window): the 3 "missing FROM-clause" events found are all timestamped 17:42Z, before this deploy (18:03:43Z) — zero new occurrences since. |
| 170 | `migrate_170_community_connection.sql` | Communities revamp part A (`docs/communities-revamp-2026-09-10/20-BLUEPRINT.md`; contract `22-MIGRATION-170A-CONTRACT.md`): `discipline_keys` taxonomy (15 keys, max 3) with validating/labelling helpers; `_community_profile_card` gains discipline fields and the nine migrate_165 consistency counters, gated on the owner's `share_consistency`/`status`/`is_minor`; `community_dimension`/`community_dimensions_me` (the latter DROPped and re-created with a required `_today`) gain `discipline`/`age_band` cohorts with member/trained-today counts and a sample, via new helper `_community_cohort_stats`; `community_board` gains scopes `area`/`style`/`discipline`/`age_band`; new `community_hub_summary(_today)`; `community_find_people` (DROPped and re-created) gains a hard `_discipline` filter. No new table; `delete_user_data()` unchanged. DEPENDS ON 160, 161, 162, 163, 164, 165, 167, 169. Parts A2 (`community_dimension_recent`, group count alignment) and B (ambient sharing with server-side consent, group audiences, notes, Together, Respect everyone, the daily Respect digest table, connect reasons) are in the SAME file. Rollback: see the file header. | **APPLIED 2026-09-10** (founder phrase; Claude-run through the Supabase MCP connection after workflow run #11 failed HTTP 401 on the repo token; checksum-verified chunks, each part executed atomically; both acceptance blocks passed; verified read-only afterwards - see the status block above). |
| 171 | `migrate_171_community_friends_trained_today.sql` | One count-only RPC `community_friends_trained_today(_today)` for the home-screen widget's friends line (communities revamp phase 4, CR-14; fresh-eyes review finding 9, data minimisation): returns the number of people the caller follows who trained on `_today`, with exactly `community_board`'s `following` eligibility and the caller excluded, capped at 999; STABLE and read-only; SECURITY DEFINER, pinned search_path, EXECUTE to authenticated only. DEPENDS ON 160, 165, 170. Rollback: DROP FUNCTION (the client's board path is unaffected). | **APPLIED 2026-09-11** (founder phrase; connector; checksum protocol; acceptance passed; verified read-only, see the status block). |
| 172 | `migrate_172_community_pr_count.sql` | Adds `community_profiles.c_prs_4w` (smallint, PRs in the last 28 days, clamped 0..200) and re-issues `community_update_training_profile` (accepts it under the share_consistency gate) and `_community_profile_card` (exposes it only when the owner shares consistency AND shares what they did) in full, carried forward byte-for-byte from 165 and 170 with only the marked lines added (guard-proved). The blueprint's "3 PRs in 4 weeks" strip figure (section 4, CR-05), never built until the 2026-09-11 review. A count, never a lift. DEPENDS ON 165, 170. Rollback: see the file header. | **APPLIED 2026-09-11** (same batch as 171; acceptance passed; verified read-only, see the status block). |
| 173 | `migrate_173_community_handle_suggestion.sql` | Community at onboarding (CR-15, D158, D159): `community_handle_suggestion(_hint)` proposes a free handle for the caller, the person's own name first (the onboarding hint, the profile first name, the provider's given name), the leading letters of the sign-in address only as a last resort (never the full local part, never returned), then `athlete`, with a bounded collision walk; two pure helpers (the sanitiser and the client's reserved-word list as an exclusion list, no client grants). VOLATILE (rate rail 30 an hour), SECURITY DEFINER, pinned search_path, EXECUTE to authenticated only. No table changes. DEPENDS ON 001, 160, 170. Rollback: three DROP FUNCTIONs (file header). | **APPLIED 2026-09-11 22:34 UTC** (connector; checksum protocol; second in the batch, after 174; verified). |
| 174 | `migrate_174_community_minor_closed_rules_v3.sql` | D159: `_community_minor(uuid)` fails closed (an unknown date of birth reads as a minor until it arrives; recomputed on every profile write; the client pushes the body profile row first) and `_community_rules_version()` returns 3 (the version-3 rules text is re-accepted once through the existing `rules_outdated` path). Two CREATE OR REPLACE FUNCTIONs, no table or grant changes. DEPENDS ON 160, 161, 170. Rollback: re-issue both from 160 and 161. | **APPLIED 2026-09-11 22:34 UTC** (connector; checksum protocol; first in the batch, before 173; verified). Part 2 rolled back to `SELECT 2` as a stopgap 2026-09-12 14:33 UTC; restored by 175. |
| 175 | `migrate_175_community_rules_gate_tolerant.sql` | D160: the rules gate in `community_upsert_profile` tolerates an older client (the version accepted is stored; the acts that need the current text stay gated by `_community_require_rules`) and a client one version ahead; `_community_rules_version()` back to 3 in the same transaction. Re-issue of 170's function with four marked changes, guard-proved. DEPENDS ON 160, 161, 170, 174. Rollback: re-issue from 170 and 174/161 (file header). | **APPLIED 2026-09-12 15:05 UTC** (founder order; connector; checksum protocol; verified). |
| 176 | `migrate_176_community_closed_groups_out_of_lists.sql` | CR-16 / D162: a closed group leaves the Hub's GROUPS section and "My groups" (`community_hub_summary`, `community_group_list_mine` keep `g.status = 'active'`), and an accepted invite returns the count after the join (`community_group_accept_invite` re-reads the group). Three CREATE OR REPLACE FUNCTIONs re-issued byte-for-byte with one marked change each, guard-proved; no table or grant changes beyond the re-stated pairs. DEPENDS ON 160, 165, 170. APPLY ORDER: 176 BEFORE 180 (2026-09-23: Part 0 added, a read-only pre-flight that refuses to re-run once migrate_180's ED gate is live in `community_hub_summary`; re-apply 180 instead, whose body carries this change). Rollback: re-issue from 170 and 165 (file header). | **APPLIED 2026-09-24 14:14 UTC** (written 2026-09-13; founder phrase given 2026-09-24; apply record in the status block). |
| 177 | `migrate_177_community_notify_recipients.sql` | Founder order 2026-09-22, item 1 ("wire the pushes"): `community_respect_all(text, text, text)` returns `{given, recipients: [{user_id, post_id}]}` (exactly the recipients given a brand-new reaction this call, never an already-respected one, never the caller) and `community_group_join(uuid)` returns `{state, admin_ids}` on the 'requested' branch only (the group's current admins), `{state}` alone on 'member'. Two CREATE OR REPLACE FUNCTIONs re-issued byte-for-byte with the marked changes only, guard-proved; no table, signature or grant changes. DEPENDS ON 160, 165, 170. Rollback: re-issue from 170 and 165 (file header). | **APPLIED 2026-09-24 14:33 UTC** (written 2026-09-22; founder phrase given 2026-09-24; apply record in the status block). |
| 178 | `migrate_178_community_note_posts.sql` | Founder order 2026-09-22, item 5 ("a first post without a workout"; audit A-05): a short free-text post kind, `'note'`, no payload keys at all, the existing `caption` field is the whole post. `community_posts_kind_check` widened to add `'note'`, keeping all five existing values. `_community_payload_keys(text)` re-issued from 160 with one marked addition (`'note'` returns an empty `text[]`, never NULL). `community_create_post` re-issued from 170 with one marked addition (a `'note'` post requires a non-empty caption, checked after cleaning). No table changes. DEPENDS ON 160, 170. Rollback: re-issue from 160 and 170 (file header). | **APPLIED 2026-09-24 14:33 UTC** (written 2026-09-22; founder phrase given 2026-09-24; apply record in the status block). |
| 179 | `migrate_179_tooling_tables_rls.sql` | Proposals item 11 (the founder's Supabase advisor email of 19 Sep 2026, `rls_disabled_in_public`): RLS enabled on the two migration-tooling ledgers `claude_schema_migrations` and `claude_seed_files`, all privileges revoked from PUBLIC, anon and authenticated. No policies (every writer is the service role or the Management API, which bypass RLS), no data touched, `IF EXISTS` throughout. DEPENDS ON nothing; safe to apply alone and first. Rollback: DISABLE ROW LEVEL SECURITY on both (file header). | **APPLIED 2026-09-24 14:04 UTC** (written 2026-09-23; founder phrase given 2026-09-24; apply record in the status block). |
| 180 | `migrate_180_community_consistency_server_gate.sql` | Founder order 2026-09-22, item 6 ("safety backstop"; audit B-02/Question 5): the calm/ED withhold on consistency sharing was enforced client-only. New helper `_community_ed_flag_open(uuid)` (fail-closed true on a null id or any error) covers the open ED-pattern flag; the calm arm reads the guarded synced pref (fact (b), file header; the mirror carries the newest edit any of the person's devices pushed). **The ED arm was DORMANT until D92-11 (Opus review 2026-09-23, H1, lead-verified): nothing wrote the cloud `ed_pattern_flags` table (the engine raises flags into device SQLite only; the sync registry has the table pull-only; no migration or edge function inserts it), so this gate, like the two edge functions' existing ED gates, withheld nothing for an engine-raised flag. FOUNDER DECISION B (2026-09-23, register D196) answers D92-11: migration 182 adds the raise-only, forward-only `ed_flag_push` write path and the client pushes on raise, clear and every sync cycle, so once 182 is applied and a device on that build raises a flag, this arm is live. The same decision adds the CALM-MODE ARM here: `_community_calm_mode_on(uuid)` reads the guarded synced pref (`user_prefs` `@volyume_wellbeing_mode` = 'calm'), and every reader now calls `_community_consistency_withheld(uuid)` = open ED flag OR calm mode, read-side only (the stored `share_consistency` is forced off by the ED flag alone, so a lagging calm mirror can never pin a stored preference off). Audit B-02 closes with 180 + 182 + the client push together.** The acceptance block probes all three helpers (NULL true, no-row false, open-row true, the OR composes; all three STABLE; the withheld helper's live body compared exactly; the calm arm probed against real pref rows in both directions when any exist, review L8) and prints the open-flag count. Six readers re-issued byte-for-byte with the marked change (`_community_profile_card`, `community_board`, `_community_cohort_stats`, `community_hub_summary`, `community_group_get`, `community_friends_trained_today`); `community_update_training_profile` re-issued to store `share_consistency = false` when the caller's flag is open, response shape unchanged. No table changes. LEAD RULING 2026-09-23 (resolving the 176/180 conflict raised in the prior draft): `community_hub_summary` is re-issued from migrate_176's body (lines 88-241, which already carries 176's `AND g.status = 'active'` closed-groups change), not migrate_170's, so applying 180 alone or after 176 lands both fixes; 176 must never (re-)apply AFTER 180 unmodified, or it reverts the ED-gate (file header, Part 5). DEPENDS ON 017, 160, 165, 170, 171, 172, 176 (REQUIRED APPLY ORDER: 176 BEFORE 180, enforced in code: 180's Part 0 refuses until 176's `community_group_list_mine` change is live, and 176's Part 0 refuses to re-run once 180's gate is live). Rollback: see the file header. | **APPLIED 2026-09-24 15:13 UTC** (written 2026-09-23; founder phrase given 2026-09-24; apply record in the status block). |
| 181 | `migrate_181_gym_moderation_lists.sql` | Founder order 2026-09-22, item 7 ("Moderation gap... a minimal moderator screen closes it"; audit `docs/audit/community-audit-2026-09-22/B-functionality-backend-safety-engineering.md` B-03/Q7). Establishes that `gyms_review_submission`/`gyms_review_report` (migrate_162, APPLIED) already check `community_is_moderator()` and already carry `EXECUTE`/authenticated with PUBLIC/anon revoked, so neither is changed. Adds the missing listing half: two new SECURITY DEFINER RPCs, `gyms_pending_submissions(_limit int, _cursor text)` and `gyms_pending_reports(_limit int, _cursor text)`, STABLE, keyset-paged (reusing `_community_cursor_parts`/`_community_cursor_of`/`_community_limit`), refusing `not_allowed` unless `community_is_moderator()`, granted to `authenticated` only. No table, no existing function, no existing grant changed. DEPENDS ON 160, 162. Rollback: two DROP FUNCTIONs (file header). | **APPLIED 2026-09-24 15:23 UTC** (written 2026-09-23; founder phrase given 2026-09-24; apply record in the status block). |
| 182 | `migrate_182_ed_flag_cloud_push.sql` | D92-11 ANSWERED, founder decision B (2026-09-23, register D196): the device now publishes its ED-pattern flag to the cloud. New SECURITY DEFINER RPC `ed_flag_push(_id uuid, _raised_at, _cleared_at, _reason)`, caller = auth.uid(); the device pushes raises and clears. Every read and write is scoped to the caller's OWN composite key `(user_id, id)` (the PK since 018; two people holding the same local id each get their own row, review M1), the stored row is locked (`FOR UPDATE`) and the insert is race-safe (`ON CONFLICT (user_id, id) DO NOTHING`); an existing row moves FORWARD ONLY (`cleared_at` set once, never nulled or moved; `raised_at` and `deleted_at` never touched; reason set once; an unchanged re-push is a no-op); clocks are clamped, never refused (a future raise or clear to now(), a clear before its stored raise up to the raise). NO SIGNALS: the function has no parameter for `signals_json` and never writes it (the detector's health-derived indicators stay on the device; nothing on a second device reads them). The owner INSERT and UPDATE policies from migrate_017 are DROPPED and INSERT/UPDATE/DELETE revoked from anon and authenticated, so the RPC is the only client write path (review L2); the owner SELECT policy stays for the existing pull. Proved on a PostgreSQL 16 harness 2026-09-24 (T1-T12: own-row scoping across two users on one id, the clamps, the no-op re-push, the refusals, service_role read, no signals, clean re-issue). Client (same landing): `pushEdPatternFlags` in `src/lib/sync/tables/edPatternFlags.js`, called after a raise or clear (`CoachOutputScreen`, guarded: never during a sign-out wipe, without Article 9 consent, without a session, or for a user other than the signed-in one) and FIRST in every sync cycle's push phase from `bulkUploadLocalData` (every open row plus rows cleared in the last 30 days); while this file is NOT applied the client skips quietly on PostgREST PGRST202 (uncounted, no Sentry error), so a build carrying the push is safe against the live server, and nothing reaches the cloud until the phrase applies it; telemetry scope neutral, no row id (review H2). The pull side ratchets (D196 item 8): a pulled clear never closes a local open row. The registry stays `pull_only`. Arms migrate_180's ED gate and both edge functions' existing ED gates. DEPENDS ON 017; independent of 176-181. Rollback: DROP FUNCTION and re-create the two policies (file header). | **APPLIED 2026-09-24 15:30 UTC** (written 2026-09-23; founder phrase given 2026-09-24; apply record in the status block). |
| 183 | `migrate_183_community_unused_rpcs_revoked.sql` | Founder order 2026-09-22, item 9 (Community hygiene), B-04: the audit named four RPCs as having zero client callers; three are true and are revoked here, the fourth (`gyms_near`) is false (a client alias, `near as nearGyms`, that the audit's own `near(` grep missed - `src/components/community/GymPicker.js`) and is untouched, proved by the acceptance block. EXECUTE revoked from PUBLIC, anon and authenticated on `community_dimensions_me(text)`, `gyms_in_place(text, integer)` and `community_gym_suggest(text, text)`; bodies untouched (no DROP FUNCTION), following migrate_164 Part 15's `FOREACH sig IN ARRAY [...]` REVOKE pattern. `community_dimensions_me` is revoked at exactly one signature: migrate_170 Part 7 already dropped the migrate_160 no-arg overload before creating the `(text)` form, and no later migration restores it. DEPENDS ON 160, 162, 164, 167, 170. Rollback: re-GRANT EXECUTE to `authenticated` on the three (file header). | **APPLIED 2026-09-24 15:33 UTC** (written 2026-09-24; founder phrase given 2026-09-24; apply record in the status block). |
| 184 | `migrate_184_community_sharing_default_on.sql` | Founder order 2026-09-26 (register D194 addendum 2: "It is on for all users by default. New and existing they can turn it off if they want after."). Part 1: `community_profiles.share_sessions` column default false to true (`sessions_audience` keeps its narrow column default on purpose, the RPC sets the audience itself). Part 2: `community_upsert_profile(jsonb, boolean)` re-issued IN FULL from 175 with two changes only: an omitted `share_sessions` on a NEW profile means ON, and an omitted `sessions_audience` means everyone for an adult and followers for a minor (minor-aware at the fallback because the clamp REFUSES everyone from a minor); an existing profile is untouched (the merge block folds its stored values back first). Part 3: `community_get_me()` re-issued IN FULL from 161 plus `share_sessions` and `sessions_audience` (NULL with no profile) so the device can mirror the row. Guard `migrate184.guard.test.js` diffs both bodies against their sources line by line. | **APPLIED 2026-09-26 13:49 UTC** (written 2026-09-26; founder phrase given 2026-09-26; first in the batch, before 185; apply record in the status block). |
| 185 | `migrate_185_community_sharing_flip_existing.sql` | Founder order 2026-09-26 ("Flip them all", register D194 addendum 2). One-off DATA change: every profile created before 22 September (the D194 default date) set to share its sessions, to everyone, or to followers for a minor (stored `is_minor` OR a fresh `_community_minor`, so a stale flag can only narrow). A profile created on or after 22 September saw the switch on before joining, so if it is off its owner turned it off and it is left alone. Read-only count 2026-09-26: two profiles, both the founder's test accounts (`alland` off/followers, `allan` on/followers), both adults; rollback by handle in the header. Idempotent WHERE; acceptance fails if any pre-default row is still off or any minor is wider than followers. | **APPLIED 2026-09-26 13:50 UTC** (written 2026-09-26; founder phrase given 2026-09-26; run after 184; apply record in the status block). |
| 186 | `migrate_186_community_sharing_never_on_unasked.sql` | Register D212 (2026-09-26, lead ruling under the founder's delegation of the same day). Found after 184 was applied: the Community Join screen created a profile without the "Share what I did" choice and published it afterwards only when it was on, while 184 made an omitted choice on a NEW profile mean on, so switching sharing off before "Create profile" would have been overridden (no profile was created in the window; read-only count: still the two test profiles). The app now sends the choice with the create; this file protects installed builds: `community_upsert_profile(jsonb, boolean)` re-issued IN FULL from 184 with one change, an omitted `share_sessions` on a NEW profile is off. The app's on-by-default (D194) is unchanged; under-18s start off (D212, ICO Children's Code standard 7). Audience fallback, minor refusal, column default and grants as 184 left them. Guard `migrate186.guard.test.js` diffs the body against 184's line by line. | **PENDING - written 2026-09-26; waits for the founder's exact phrase "run against production" naming it.** |
