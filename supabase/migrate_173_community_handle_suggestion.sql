...
--                    person's username or email local part, then delegated
--                    the data-minimisation question (Q2) to the lead on the
--                    "best app" criterion. RULED (D159, 2026-09-11): the
--                    person's own first name comes first and the email only
--                    as a last resort, and then only its LEADING LETTERS
--                    (up to the first dot, underscore, plus, dash or digit),
--                    never the full local part. So `john.smith83@` becomes
--                    `john`, not `john_smith83`: a public handle should not
--                    publish a full name and a birth year the person never
--                    chose to publish. `email` is on Community's refusal
--                    list and the privacy source guard forbids every
--                    Community client file from reading it, so the
--                    derivation happens HERE, inside a SECURITY DEFINER
...
--
--                    Sources, in order: `_hint`, the first name the person
--                    typed at onboarding step 2, passed by the onboarding
--                    step alone (the profile is not on the cloud until
--                    setup completes, so at that moment only the client has
--                    it; sanitised like everything else, never stored);
--                    the first name on the account's own profile
--                    (`users_profile.first_name`, the name the person gave
--                    the app), for the Join screen and every retry; the
--                    given name the sign-in provider supplied
--                    (`raw_user_meta_data.given_name`, else the first word of
--                    `full_name` or `name`); the leading letters of the
--                    email local part, unless the address is an Apple
--                    private relay (`privaterelay.appleid.com`, a random
--                    token that names nobody); the neutral base `athlete`.
...
--                    Oracle bound (hostile review 2026-09-11, re-ruled after
--                    the D159 amendment): `_hint` puts the base in the
--                    CALLER's hands, so the bound is the RAIL alone: thirty
--                    suggestions an hour, each revealing whether one chosen
--                    base and its `_N` neighbours are taken. Strictly weaker
--                    than community_check_handle's 120 free-form checks an
--                    hour on its own separate rail; the two together are
--                    150 an hour, which is still a person typing. Thirty an
--                    hour covers a screen mounting.
--
...
-- Applied remotely:  YES - 2026-09-11 22:34 UTC, SECOND in the batch (after
--                    174, which the client's rules version depends on), under
--                    the founder's exact phrase "run against production"
--                    given 2026-09-11; Claude-run through the Supabase
--                    connector under the checksum protocol (supabase/
--                    README.md status block): two chunks verified md5
--                    3e2ef5b4168934df44b819b7f604a757 / 11,107 bytes and
--                    0c979c6500729e35826d0b05c948aaa5 / 9,241 bytes, whole
--                    file 7010e656ae202a9b27e710c06772a636 / 20,348 bytes
--                    (the file as applied, before this line was updated),
--                    re-checked inside the executing DO block; acceptance
--                    block passed; verified read-only afterwards (the one
--                    (text) signature, VOLATILE, executable by authenticated
--                    and not anon, both helpers IMMUTABLE and executable by
--                    no client role, every community function pinned). The
--                    client treats a missing function as `unavailable`, so
--                    an older build degrades to an empty handle field.
-- Safe to re-run:    YES - CREATE OR REPLACE FUNCTION, REVOKE/GRANT are
--                    idempotent, and the acceptance block is read-only.
-- Rollback:          DROP FUNCTION IF EXISTS public.community_handle_suggestion(text);
--                    DROP FUNCTION IF EXISTS public.community_handle_suggestion();
--                    (the zero-argument name the acceptance block refuses,
--                    dropped too so a rollback leaves nothing behind)
...
-- Depends on:        001 (users_profile.first_name), 160 (community_profiles,
--                    _community_caller, _community_rate_check,
--                    _community_handle_valid, _community_handle_reserved),
--                    170 (community_upsert_profile creates the profile the
--                    suggestion is for).
...
CREATE OR REPLACE FUNCTION public.community_handle_suggestion(_hint text DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
BEGIN
  ...
  -- Source 1: the first name the person typed at onboarding, passed by the
  -- onboarding step as `_hint`. Sanitised like everything else; a hint is
  -- a base to derive from, never stored, never echoed.
  v_base := public._community_handle_base(left(coalesce(_hint, ''), 60));
  IF v_base IS NOT NULL THEN v_source := 'name'; END IF;

  -- Source 2: the first name on the account's own profile, the name the
  -- person gave the app (synced), for the Join screen and every retry.
  IF v_base IS NULL THEN
    SELECT nullif(btrim(coalesce(up.first_name, '')), '') INTO v_given
    FROM public.users_profile up
    WHERE up.id = v_uid;
    v_base := public._community_handle_base(left(coalesce(v_given, ''), 60));
    IF v_base IS NOT NULL THEN v_source := 'name'; END IF;
  END IF;

  -- Sources 3 and 4 need the one read of the caller's own sign-in record,
  -- taken only when the name sources gave nothing. The address is held in
  -- a local for the two split_part calls and is never returned, logged or
  -- raised.
  IF v_base IS NULL THEN
    ...
    -- Source 3: the given name the sign-in provider supplied, when any.
    ...
    -- Source 4 (Q2, data minimisation): the LEADING LETTERS of the email
    -- local part only, never the full local part, and never from an Apple
    -- private relay, whose local part is a random token.
    IF v_base IS NULL AND v_domain <> 'privaterelay.appleid.com' THEN
      v_base := public._community_handle_base(substring(v_local FROM '^[a-z]+'));
      IF v_base IS NOT NULL THEN v_source := 'email'; END IF;
    END IF;
  END IF;

  -- Source 5: a neutral base.
  IF v_base IS NULL THEN
    v_base := 'athlete';
    v_source := 'fallback';
  END IF;
...
REVOKE ALL ON FUNCTION public.community_handle_suggestion(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.community_handle_suggestion(text) TO authenticated;
...
DECLARE
  v_vol "char";
BEGIN
  IF to_regprocedure('public.community_handle_suggestion(text)') IS NULL THEN
    RAISE EXCEPTION 'acceptance failed: community_handle_suggestion(text) missing';
  END IF;
  IF to_regprocedure('public.community_handle_suggestion()') IS NOT NULL THEN
    RAISE EXCEPTION 'acceptance failed: a zero-argument community_handle_suggestion overload exists';
  END IF;
  ...
  FROM pg_proc p WHERE p.oid = to_regprocedure('public.community_handle_suggestion(text)');
  ...
  IF NOT has_function_privilege('authenticated', 'public.community_handle_suggestion(text)', 'EXECUTE') THEN
    RAISE EXCEPTION 'acceptance failed: community_handle_suggestion is not executable by authenticated';
  END IF;
  IF has_function_privilege('anon', 'public.community_handle_suggestion(text)', 'EXECUTE') THEN
  ...
  END IF;
  -- Source 4 takes the leading letters only (Q2): a full local part never
  -- becomes a handle through that route.
  IF public._community_handle_base(substring('sam.j.parker-1990' FROM '^[a-z]+')) IS DISTINCT FROM 'sam' THEN
    RAISE EXCEPTION 'acceptance failed: the email route does not stop at the first separator';
  END IF;
  IF public._community_handle_base(substring('sj.parker' FROM '^[a-z]+')) IS NOT NULL THEN
    RAISE EXCEPTION 'acceptance failed: a two-letter email prefix must give nothing';
  END IF;
  IF public._community_handle_base(substring('1990parker' FROM '^[a-z]+')) IS NOT NULL THEN
    RAISE EXCEPTION 'acceptance failed: a local part starting with digits must give nothing';
  END IF;
