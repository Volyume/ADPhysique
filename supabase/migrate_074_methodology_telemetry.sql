-- Migration 074: add the COMP-006 methodology-open event to
-- record_engine_telemetry.
--
-- COMP-006 ("How Precision Coaching works"). One event measures trust
-- formation:
--
--   methodology_opened   (MethodologyScreen mount)
--     payload: { source } — why_block | held_decisions | you_tab | paywall
--
-- No PII: an entry-point label only, never training or body data. Flows
...
-- the row re-pushes forever. Reproduces the migration 073 list verbatim plus
-- the one new name.
--
-- Applied locally:  NO (pending)
-- Applied remotely: NO (pending founder apply)
-- Safe to re-run:   YES. CREATE OR REPLACE; no schema change.
-- Rollback:         re-apply migration 073 to drop the name.
