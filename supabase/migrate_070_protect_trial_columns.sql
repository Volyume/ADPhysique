...
-- ════════════════════════════════════════════════════════════════════
--
-- Problem (trial/subscription forensic audit 2026-06-08, finding C1):
...
--     - trial_state = 'unstarted' -> then start_cascade() (or the
--       ProUpgrade "start your free trial" path) grants a fresh 14-day
--       trial, violating the one-time-trial rule
--       (COMPLETE_TIER_SCOPE_LOCKED.md lines 91-94).
--   Either path is unlimited free Pro for anyone who can sign a request.
--   The prior security audit cleared "a user cannot forge Pro" but only
--   inspected the `tier` column (05-security-audit.md lines 52-55).
--
-- Fix:
