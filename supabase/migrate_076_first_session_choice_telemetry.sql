-- Migration 076: add the COMP-013 first-session-choice event to
-- record_engine_telemetry.
--
-- COMP-013 (plan reveal moment + 15-minute starter session). One event
-- measures the activation choice a brand-new Pro user makes on the Home hero
-- first-run variant:
--
--   first_session_choice   (Home hero first-run variant tap)
--     payload: { choice } — short | full
--
-- No PII: a choice label only, never training or body data. Flows through the
-- same opt-out gate as every other event.
--
...
-- the row re-pushes forever. Reproduces the migration 075 list verbatim plus
-- the one new name.
--
-- Applied locally:  NO (pending)
-- Applied remotely: NO (pending founder apply)
-- Safe to re-run:   YES. CREATE OR REPLACE; no schema change.
-- Rollback:         re-apply migration 075 to drop the name.
