...
===============================================================================
## ★ 2026-09-26 (evening) — RECOVERY LEARNING LANDED AFTER TWO REVIEWS (D210 ADDENDA 3 TO 5); FOUNDER: NO BUILDS WITHOUT THEIR GO; PLAIN-ENGLISH RECOVERY CARD ★

Sources: `docs/TASKBOARD.md` section 1 (the recovery learning line), `docs/ux-world-class-audit-2026-07-09/DECISIONS-2026-07-09.md` D210 addenda 3, 4 and 5, `docs/recovery-programme-2026-09-25/14-PERSONAL-LEARNING-V2.md` section 0. Founder, verbatim: "No finish all tasks and push to main. I'll do a build when I am ready you do not have permission to push builds"; "Remember the copy rules at all times, understandable plain British English"; "Do the tasks I asked you to do ... Stop fucking around ok things I don't ask for" (usage is short: do only what is asked). Done later the same evening on the founder's word (D204 addendum 4): 186 applied 18:44 UTC, the held safety lines reworded. Open, founder-side: a build and its device walk; 155 applied the same evening ("run against production: 155"); the store data-safety forms. The review's N3 and N4 done the same day (D210 addendum 5).

===============================================================================
## ★ 2026-09-26 — PLAIN-ENGLISH SWEEP (D207) LANES A TO E ON THE BRANCH; SHARE IMAGE REDESIGN (D209), RECOVERY'S OWN PLACE (D208), TODAY'S WEIGH-IN MOVE AND THE D204 ADDENDA ON MAIN; LANE F (D204 BREACHES) RUNNING; MIGRATIONS 184 AND 185 APPLIED 13:49 AND 13:50 UTC ★

Sources, in order: `docs/TASKBOARD.md` section 1 (every lane's hash and state), `docs/ux-world-class-audit-2026-07-09/DECISIONS-2026-07-09.md` D204 to D209 (every ruling and its rationale), `docs/rules/plain-english.md` (the sweep's rule). Position: main carries the share image redesign `28ae0332` and everything before it; the branch `claude/communities-feature-overhaul-68l923` carries Lane D `9011b6ef`, the plan-preview payment line `a4546d7b`, the share-font fallback `19f68b56`, What's new `ac2266d5` and the docs commits, gated or gating in `scratchpad/gate-wt` before each push to main. Lane F (Opus) fixes the D204 breaches Lane D listed (`src/lib/coachingGoals.js`, `blockAdvisor.js`, `readinessSummary.js`, `home/todayLineArbiter.js`, `CoachReviewScreen.js`, `volumeInsightCopy.js`, `reEntryCheck.js`, three "aim to" lines) and the last collaborative "we" lines; anything tied to what a button does is stopped for the lead. Recovery path if Lane F dies: its uncommitted diff is lead-reviewed string by string against D204 and its brief, the sound strings land, and it is relaunched on the rest. APPLY RECORD 2026-09-26: the founder's phrase "run against production" was given 2026-09-26 with "Make the decisions that bring the absolute best product for end users ... I do not need to make these decisions" (D33 delegation of every open product call; the Section 2 inviolables still bind, and ED-safety wording stays STOP-and-ask). Migrations 184 (13:49:56 UTC) and 185 (13:50:46 UTC) ran through the Supabase connector under the checksum protocol; both acceptance blocks passed; verified read-only after: `share_sessions` default on, both test profiles on/everyone, `community_get_me` returns the setting, 175 community functions pinned and none anon-executable; the scratch table dropped 13:51 UTC. Record: `supabase/README.md` status block, entry "184 AND 185 APPLIED 2026-09-26". Founder-side, still open: the store data-safety answers; creating 2.4.0 in App Store Connect before the next iOS upload; the ED-safety wording list (STOP-and-ask under CLAUDE.md even under delegation). Delegated and now lead calls: the website deploy, the under-18 default, the share-image extras, the Welcome screenshots (rendered by Claude Code, per the founder: re-render from the current app). No build has been dispatched.

===============================================================================
## ★ 2026-09-24 — COMMUNITY PROPOSALS 1 TO 11: ITEMS 1 TO 9 ON MAIN (`590b03fd` migration 180, `b4a24e65` migration 181, `cbdbf8be` item 8, `49dfc0f3` decision B, `c10b85f0` item 9), THE NUTRITION ORDER (`26cf33cc`) AND MIGRATION 179 (`2e567408`) TOO; FOUNDER DECISION B ON D92-11 (D196) LANDED AFTER THE OPUS REVIEW; ITEM 9 HYGIENE LANDED (D197: MIGRATION 183, THE NOTIFY PROOF BOUND, THE RETIRED PAGE, THE TESTS, THE CENSUS); 176 TO 183 APPLIED 2026-09-24 UNDER THE PHRASE (14:04 TO 15:33 UTC, 176 BEFORE 180; community-notify VERSION 4 AT 15:38 UTC); NEXT: THE PROGRESS-TAB AUDIT (FOUNDER ORDER 2026-09-24: VOLUME HEATMAP, BODY HEATMAP, CONSISTENCY, THIS WEEK'S PLAN, RECOVERY), THEN ITEMS 10 AND 11 (FOUNDER-SIDE) ★

APPLY RECORD 2026-09-24: the founder's phrase "run against production: 176, 177, 178, 179, 180, 181, 182, 183" was given 2026-09-24 and all eight ran through the Supabase connector under the checksum protocol (per-chunk md5 into a scratch table, whole-file md5 re-checked inside the executing DO block, tracking row, read-only verification; the scratch table dropped after). Timestamps, checksums and verification per file: `supabase/README.md` status block, entry "176 TO 183 APPLIED 2026-09-24". The community-notify edge function is at version 4 (15:38 UTC; the note push copy and the group_accepted recency bound), fetched back and compared to the repo. 182 is live, so the ED arm of migration 180 and both edge-function ED gates arm on the first device push; a build carrying the push has not shipped and needs the founder's go. PROGRESS-TAB AUDIT, FIRST PASS DONE 2026-09-24: `docs/audit/progress-tab-audit-2026-09-24/00-FINDINGS-AND-PROPOSALS.md` is the source (F1 to F7, P1 to P7, Q1 to Q4), register D199. F2 FIX LANDED 2026-09-24 (This week's plan counts the block's own week on Consistency and Home through one helper, end-to-end test; board carries the file list). LANE B LANDED 2026-09-24 `d2b92e4e` (F10 to F14 on Lift progress and Exercise detail; the board carries the file list and the gate figures). The founder answered Q1 to Q4 with "you make the call" and "fix it all" (register D200), and the R5b findings are in the report's section 5. LANE A LANDED 2026-09-24 `04e82961` (F8, F9, F15, F16, F17 on Body metrics and Workout history; the board carries the file list and the gate figures). LANE C LANDED 2026-09-25 `830b73de` (F1 and F6, the heatmap windows read a weekly average; the board carries the file list and the gate figures). LANE D LANDED 2026-09-25 `b117bb06` (F3, the Recovery block; the board carries the file list and the gate figures). The section-6 read lane's seven findings are in the report's section 7; the calm-mode one landed by the lead's hand (`19e39260`), five landed through fix lane S6 (`51849a74`), the last goes with lane E. LANE E LANDED 2026-09-25 `cd9269af` (F4, F5, S6-5: one definition of week per meaning, one load surface; the board carries the file list and the gate figures). The section-6 read lane (S6) and the photo/scan read lane (S7) are done; their findings are in the report's sections 7 and 8; S6-1, S7-1 and S7-5 landed by the lead's hand, S6-2/3/4/6/7 through fix lane S6, the athlete-profile keying through fix lane AP. FIX LANE S7 LANDED 2026-09-25 `0fdec1c9` (the scan chain, Compare, the share sheets, the seven-day weigh-in bound). THE PROGRESS-TAB AUDIT IS COMPLETE: every finding in the report's sections 1 to 8 is on main; the board carries every hash and gate figure. NEXT PROGRAMME (D201, 2026-09-25): per-muscle recovery estimate, next-workout aware, recovery-sequenced plans; the spec is `docs/recovery-programme-2026-09-25/00-SPEC.md`; the founder's go was given 2026-09-25 and forks F1 to F3 are lead-ruled (register D201 addendum, plus addenda 2 to 4 for the review rulings). BUILD IN FLIGHT: step 0 `647788e9`, R-A `99ee7354`, R-B `ed17119c`, R-C `0ffd7269` and R-D `00e78164` are on main; the Opus fresh-eyes review returned 25 findings and every one is fixed by the lead's hand (register D201 addendum 6, rulings 11 to 16; the board carries the file list, hashes and gate figures); the device checklists were delivered in chat. The review's finding 25 (test guards only) landed through lane R-G. THE RECOVERY PROGRAMME (D201) IS COMPLETE ON MAIN. RESUME: no build lane is open; the board line carries every hash and gate figure. The founder's 2026-09-26 TestFlight walk of Consistency produced four fixes, all on main (D204: surfaces never tell the athlete to change a session; the D200-2 addendum that reordered the Recovery section and hid the dials was withdrawn the same day on the founder's word and the order is restored; D201 addendum 7: the recovery percent is relative to the session's own peak; D201 addendum 8: compact per-muscle rows, name, bar, percent and one meta line; the block name wraps). The app version is 2.4.0 as of 2026-09-26 on the founder's instruction (2.3.0 was burned; 2.2.0 before it). The Coaching decision screen was redesigned 2026-09-26 on the founder's order (register D206, the redesign addendum; preview https://claude.ai/artifact/Sjq98WMVwpBirMkNYYaqgp). Next founder-side action is a build from main (nothing dispatched; the founder's go is required per the 2026-09-04 rule), then the device walk of the 19-step checklist delivered in chat. The three noted cleanups landed 2026-09-25 on the founder's "Do the items on the board" (register D202 for the age input; the board carries the hash and gate figures). Noted, not started: `buildWeeklySessionCounts` (`progressSeries.js`) has had no caller since Campaign 23 and is dead code. Next founder-side action is a build from main (nothing dispatched; the founder's go is required per the 2026-09-04 rule), then the device walk of the checklists delivered in chat. Two small cleanups are recorded on the board and not started: retiring the now-unused `getAcuteChronicWorkload`, and the drifted line numbers `docs/_FULL-APP-PRODUCT-MAP.md` cites for `chartWindows.js`. Each lane lands as its own lead-reviewed commit over a green tree; the board line records every hash.

Resume from `docs/TASKBOARD.md` (the section at the top and section 3) and the register: D195 items 9 to 11 (items 6, 7 and 8, including the review finding that nothing wrote the cloud `ed_pattern_flags` table) and D196 (the founder's decision B and the nine lead rulings under it: one narrow RPC write path scoped to the caller's own row with clocks clamped, no signals leave the device, owner write policies dropped, the calm arm read-side only, why the prefs mirror is sound to read and what it is (the newest edit, not the strictest), what becomes live, the record corrections, the pull ratchet (a pulled clear never closes a local open flag), and the review's fixes). The decision B diff on main: `supabase/migrate_182_ed_flag_cloud_push.sql`, the `migrate_180` amendment (calm arm, L8 acceptance), `migrate_176` Part 0 needle, their three guards, `src/lib/sync/tables/edPatternFlags.js` (push, neutral telemetry, PGRST202 quiet skip, guarded immediate push), `src/lib/database.js` (`getEdPatternFlagsForCloudPush` with the 30-day window; `upsertEdPatternFlagFromCloud` ratchet), `src/lib/sync.js` (`bulkUploadLocalData`, push first in the push phase, counted), `src/screens/CoachOutputScreen.js`, `src/lib/observability/sentryScrub.js`, the push test, the pull-ratchet test (real database.js on in-memory SQLite) and the contract-test re-anchors. The RPC and the amended 180 helpers were proved on a PostgreSQL 16 harness (scripts and outputs in `scratchpad/pg/`, scenarios T1-T12 and the L8 checks with a negative case). Item 9 landed 2026-09-24 (register D197, eight rulings: three RPCs revoked not four, one `community_dimensions_me` signature, a revoked RPC leaves the security-matrix inventory, the `group_accepted` recency bound, the static retired `public/p` page, the new tests, the census extensions, not a build gate). D198 (2026-09-24, lead hands-on after the founder asked why work had stopped): every training-profile sender composes one gated payload (`composeTrainingProfilePayload`), and `saveUserBodyProfile` keeps any field a caller leaves undefined, so a wizard re-run no longer nulls the SCOFF score. Items 10 and 11 are founder-side: Partners migration 155 waits for the store apps to move to a build without Partners; the leaked-password dashboard setting and the two dashboard-only edge functions need the founder's word; the structured questions are in chat. Lanes are two at a time, supervised by an eight-minute check-in. Nothing is applied to production until the founder's phrase.
