...
### SCAN-ACC-1: Progress Scan accuracy round (founder order 2026-07-13 "when I next do a round of fixes I want it improved") — **IN FLIGHT 2026-09-25** (founder answer to the lead's fork after the three board cleanups: "Both, in board order", so SCAN-ACC-1 first, then CP-10 batches F onward). Stage 1 DONE (SR1 and SR2, 2026-09-25); stage 2 DONE: **SPEC WRITTEN `docs/scan-accuracy-2026-09-25/00-SPEC.md`** (the evidence, the design: JavaScript owns the geometry through a new pure `progressScanRaster.js`, native shrinks to `decodeOriented` with explicit sRGB and one integer box filter specified to the bit, two-pass zoom in JavaScript, median of three frames, the side pose in confidence, no calibration table until a founder parity check proves a residual offset, a measurement-version and score-version boundary so old and new scans are never compared; validation contract; lanes SA-0 to SA-4). The four score-changing audit decisions D1 to D4 are the founder's (spec section 3, F1 to F4), asked in chat 2026-09-25. Stage 3: SA-0 (the raster module, lead hands-on) is unblocked and starts while the answers are open; SA-1 to SA-3 follow the answers. Recovery path: relaunch a dead read lane from this entry's END STATE list and D84/D85; nothing is written until the spec exists.
