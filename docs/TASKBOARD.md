...
**COMMUNITY SHARING ON FOR EVERYONE, founder order 2026-09-26 ("Flip them all and I test with all ... It is on for all users by default. New and existing they can turn it off if they want after. We need that in the privacy policy also") — BUILT, MIGRATIONS WAIT FOR THE PHRASE** (lead, hands-on; register D194 addendum 2): the privacy policy (in-app, the hosted pages, the Markdown source, the docs/web mirror) gains a Community section and loses its "no community" claim, with a guard (`9aa9cba0`); `migrate_184` (column default on, new-profile fallbacks on and minor-aware, `community_get_me` returns the setting) and `migrate_185` (flip every pre-22-September profile, today the founder's two test accounts) written with a line-by-line guard, both PENDING the phrase (`9aa9cba0`); the phone mirrors the server's setting and the summary names a refused post (`5a7dfcc0`). Open with the founder: the phrase for 184 and 185, the volyume.app deploy (not run since 1 September; also switches on four iOS app-link paths), an in-app policy-change notice, the store data-safety answers, the under-18 default, the lawful basis.
