...
**COMMUNITY OPEN QUESTIONS RULED, AND A SHARING DEFECT FIXED (register D212, lead, 2026-09-26, under the founder's delegation)** — volyume.app DEPLOYED (run #81, verified live: free wording, the Community privacy section, the link pages; the iOS app-link paths inert, no entitlement); the in-app policy notice is 2.3.0's first What's New line; under-18s start with "Share what I did" OFF (ICO Children's Code standard 7); lawful basis as the policy states. DEFECT: the Join screen omitted the sharing choice on create and published it only when on, so since 184 an "off" at Join would have been stored on (no profile created in the window). FIXED ON THE BRANCH: the choice travels with the create; migration 186 (an omitted choice on a new profile is off again) WRITTEN, PENDING THE FOUNDER'S PHRASE. Founder-side: the store data-safety answers (Play: written out in `docs/PLAY_STORE_LISTING.md`, re-derived from the code 2026-09-26; App Store: a Sonnet read lane maps the same list onto Apple's categories, checking each item in the code, into the scratchpad for delivery in chat; recovery path: relaunch it with the same brief, it writes nothing in the repo).
