...
### SCAN-ACC-1: Progress Scan accuracy round (founder order 2026-07-13 "when I next do a round of fixes I want it improved") — **IN FLIGHT 2026-09-25** (founder answer to the lead's fork after the three board cleanups: "Both, in board order", so SCAN-ACC-1 first, then CP-10 batches F onward). Stage 1: two read lanes (Sonnet) dispatched, SR1 the native modules and the vision pipeline (the six end-state seams, determinism guards), SR2 the calibration telemetry, harness and safety isolation; stage 2: the lead writes the spec from their file:line evidence with the forks for the founder (one already known: any calibration pass from production telemetry reads users' scan rows, so which rows may be read is the founder's call); stage 3: build lanes after the go. Recovery path: relaunch a dead read lane from this entry's END STATE list and D84/D85; nothing is written until the spec exists.
