...
**RECOVERY LEARNS EACH PERSON'S RECOVERY, founder question 2026-09-26 ("We had recovery intelligence that learns people's recovery and adjusts as it goes along based on performance from a start. Is that what you've used for the recovery section or have you used rudimentary numbers?") and order ("We ship all we don't hold things back ... we need an elegant way to show and demonstrate this intelligence too") — FIRST BUILD `04c92d66` WITHDRAWN AFTER ITS REVIEW (D210 addendum); REBUILT AND CALIBRATED, UNDER FRESH REVIEW BEFORE LANDING** (lead, hands-on; register D210 addendum 2; spec `docs/recovery-programme-2026-09-25/14-PERSONAL-LEARNING-V2.md` section 0 "As built"): one recovery speed per person, starting at the "How's your recovery?" answer, learned from effort-matched pairs of the same lift (continuous outcome, bounded fit, outlier guard, injury limits and return periods excluded), moved only past a gate set by the full calibration (`PERSONAL_LR_MIN` 12, 600 simulated athletes a cell: a direction shown to at most 2.5% of people whose recovery equals the start, the wrong direction to at most 1.2%; about 3 in 10 slow recoverers found in twelve weeks on varied or three-day schedules; the progression over a longer break fitted per day so it cannot read as recovery; plan users on fixed days see "Not learning yet" with the reason); daily memo in `load.js`, about 3 to 7 ms per person in node. Shown by the new "Your recovery speed" card under Recovery by muscle (a faster-to-slower scale, first estimate and you, two tiles in days, the evidence count, and the reason when it cannot say yet) and in the caption and each muscle's breakdown once in use; a 2.3.0 What's New line. Recovery tests 12 suites, 231 tests; card and wiring 43 tests; lint clean. An Opus fresh-eyes adversarial review is RUNNING (read-only; recovery path: if it dies, relaunch with the same brief over the uncommitted recovery files). Lands on main after its findings are dealt with and the full gate. Then the store set is re-rendered (task below).

**WELCOME CAPTURES RENDERED FROM THE CURRENT APP, founder 2026-09-26 ("I did not create the screenshots. Claudecode rendereed then itself") — DONE ON THE BRANCH** (lead, hands-on; register D211): the paper-render harness gained a Welcome pass (phone-height layout with the real tab bar, clean account state through the offers' own dismissal keys), real Skia rings (recorded paths drawn as SVG) and meal-time food logs; `welcome.js` shoots and encodes the three captures at 480 x 940; `assets/welcome/` replaced. STORE LISTING SET: the harness now renders it (`4d3b8274`: nine captures at Play 1080 x 1920 and App Store 1290 x 2796, PNG, into scratch; Sonnet lane, lead-reviewed). The first set's Recovery capture opened on three "N/A" dials (the persona had no ratings). v2 BUILT (lead, hands-on, lands with the recovery learning): the persona gains rated sessions and eleven weeks of history before its block, generated by the recovery model itself at a faster-than-average speed (0.75, 12% sensitivity), which the learner then finds on its own through the app's loader (the store pass asserts it: 194 comparisons, "Faster than first estimated"); a phone shot can start at a named section (`scrollToText`), so Recovery is shot twice (the muscle figure, and the "Your recovery speed" card); and a converter bug that drew the body figure's back view on top of its front (a `<G x>` a browser ignores) is fixed. Ten captures a platform; Play takes eight, so the founder picks. Delivered to the founder once on main; the uploads are founder-side.
