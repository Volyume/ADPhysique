...
**RECOVERY LEARNS EACH PERSON'S RECOVERY, founder question 2026-09-26 ("We had recovery intelligence that learns people's recovery and adjusts as it goes along based on performance from a start. Is that what you've used for the recovery section or have you used rudimentary numbers?") — FIRST BUILD `04c92d66` WITHDRAWN AFTER ITS ADVERSARIAL REVIEW (DO NOT LAND: noisy dips, false guarantees, untrue copy, cost; D210 addendum), code off the branch; REBUILD IN PROGRESS to the addendum's requirements** (lead, hands-on; register D210, spec `docs/recovery-programme-2026-09-25/00-SPEC.md` section 14): answered in chat with evidence (the estimate used fixed research baselines; the app's learners moved only set counts). Now a per-muscle factor starts at the recovery answer's and is fitted to how the athlete's lifts went when a muscle was trained again, checked on the change the estimate predicts between two sessions trained to the same RIR target; recovery weeks, circuits, injury limits and mixed results never teach; pure, replayed from 126 days of sessions, bounded, three checks minimum; each muscle's breakdown says how it was adjusted. Recovery tests 192 of 192 and the UI suites green; lint and typecheck clean. An Opus fresh-eyes adversarial review is RUNNING (read-only; recovery path: if it dies, relaunch with the same brief against `04c92d66`). Lands on main after its findings are dealt with and the full gate.

**WELCOME CAPTURES RENDERED FROM THE CURRENT APP, founder 2026-09-26 ("I did not create the screenshots. Claudecode rendereed then itself") — DONE ON THE BRANCH** (lead, hands-on; register D211): the paper-render harness gained a Welcome pass (phone-height layout with the real tab bar, clean account state through the offers' own dismissal keys), real Skia rings (recorded paths drawn as SVG) and meal-time food logs; `welcome.js` shoots and encodes the three captures at 480 x 940; `assets/welcome/` replaced. Store listing screenshots stay founder-side (console uploads); the harness can render a store set on request.
