...
**In flight (launched 2026-09-24 ~11:45 UTC):** item 9 hygiene, two Sonnet lanes from `scratchpad/briefs/item9-hygiene.md`, split by FILE so they cannot collide. Lane A (server + gyms lib): Part A migration 183 + its guard + README + matrix targets + the `inPlace`/`gymSuggest` deletions + the census re-anchors; Part C's gyms wrapper tests (`src/lib/gyms/__tests__/index.test.js`, the file Lane A already edits); Part D the community-notify `group_accepted` recency filter + guard. Lane B (pages, screen tests, census): Part B the retired static `public/p` page + its pins; Part C's `CommunityGroupMembers` screen test; Part E the copy census extensions. Recovery path: each part is independently landable; a dead lane's partial tree is lead-reviewed hunk by hunk against the brief's Part text (every Part names its files and its pins) and finished by hand or relaunched for the unfinished Parts only; nothing is committed until lead review and the full gate. **Queued, in order:** item 9 hygiene (brief drafted at `scratchpad/briefs/item9-hygiene.md`: migration 183 revokes EXECUTE on `community_dimensions_me`, `gyms_in_place` and `community_gym_suggest` ONLY, since the audit's `gyms_near` zero-caller claim is wrong, `GymPicker.js:58` imports it as `nearGyms`; the static retired page for `public/p`; tests for the gyms `submit`/`confirmSubmission`/`report` wrappers and `CommunityGroupMembersScreen`; the `group_accepted` recency filter in community-notify; the copy census extended to `src/lib/community`, the nested-template-literal case and an "Age band" census, from the item 8 review); item 10 Partners migration 155 (founder confirmation and phrase); item 11 remainder (leaked-password setting in the dashboard; the two dashboard-only edge functions "tips" and "bright-handler").
