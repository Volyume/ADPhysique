...
**PLAIN-ENGLISH SWEEP, founder order 2026-09-26 ("We need a sweep and a correction on any language like this. It needs to be understanding") — IN FLIGHT** (register D207; the rule is `docs/rules/plain-english.md`, which defers to the locked voice doc's actor rule: "your coach" or impersonal, never a collaborative "we" for a coaching decision). Lane A (Opus): the coaching copy, `coachStory.js`, `coachRegister.js`, `coachResponse.js`, `coachOutput/viewCopy.js`, `weeklyCoach.js` (string literals only), `whyThisTemplates.js` (non-ED), `coachLedger.js`, `homeCoachBrief.js`, `coachDecline.js`, `coachPrecedence.js`, `capability/reintroduction.js`, `capability/weekNote.js`, `blockExplain.js`, the Coaching decision, Coaching history, Weekly check-in, You and Methodology screens, `components/coachOutput/`. Lane B (Sonnet): Today, the workout and summary screens, Plan detail, Consistency, Analytics, Body metrics, Lift progress, Exercise detail, `src/components/` outside the coach output, community and food folders, `lib/recovery/`, `chartWindows.js`, `trainingRecency.js`, `progressSeries.js`, `trainingLoad.js`. Lane B LANDED `75cfaee1` (19 lines, one caption changed by the lead, Settings, Coaching brought into line). Lane A LANDED `838ffb5a` (about 180 lines across the engine's sentences and the coaching screens; the lead fixed four untrue lines it found, the D204 Today brief, the recovery-week note and the joint-pain guidance). Lane C (nutrition, including the weight-trend lines) LANDED `373e7006` (plus the Body-row pins `8e89f918`). Lane E (settings, onboarding, notifications, community) LANDED `aa00fe41`, with the lead fixing the three coaching-mode lines its pinned test kept from it and the Consistency tooltip that named Volyume as the decider. Lane D (plans, programmes and the rest of the coaching copy) LANDED `9011b6ef` (63 files; the lead fixed a two-measure outcome line that could claim neither measure moved). Lane D listed the D204 breaches still on plan and Today surfaces (the early-deload card, the peak and recovery week lines, the weekly training notes, the readiness chip, the coach review page, the volume "Why?" panels, re-entry, three "aim to" lines) and the last collaborative "we" lines: Lane F (Opus) on exactly those, describe-only: LANDED ON THE BRANCH `1c8f3f53` after lead review string by string, with its three stopped items ruled by the lead (one "Got it" button on the early-deload card; the Training review's "What to focus on next week" section retired; the volume "Why?" panels describe instead of advising; D204 addendum 3), plus two untrue lines it found fixed `ece8d514` (the re-entry note when no session is bound; the sleep chip's "averaging"). Lane G (Sonnet) RUNNING on the rest it found (non-ED nutrition "we" lines, `algorithms.js` 959, `insightsEngine.js` 189 and 217, `EngineLog.js` 53); recovery path as below. For the founder (never edited under delegation): the ED-safety "we" lines (`whyThisTemplates.js` hold and rapid-loss, `NutritionTargetsScreen.js` 1624, `CalorieBankSheet.js` 29), `SubscriptionPolicyScreen.js` 89, and `weeklyCoach.js` 1932 ("fewer sets this week" while its Apply sets next week's recovery week; the file is on the do-not-touch list). The check-in reminder's locked line was amended by dated amendment (`4ac56633`). The founder's Coach-tab reports ("the exact signals behind it", "Your own numbers take precedence", "Sort the entire You page", "weekly loop") went to Lane A verbatim with every YouScreen line inventoried. The sweep found two D204 breaches on Consistency (the fatigue card's line, the "Lighter week recommended" banner): FIXED `7d7e423c`, D204 addendum 2. Recovery path for every lane: agents never commit; if a lane dies, its uncommitted diff is lead-reviewed string by string against the rule and its brief, the sound strings land, and the lane is relaunched on its remaining files with the same brief. ED-safety, legal, consent and locked-surface strings are listed for the founder, never edited.

---

**SHARE IMAGE REDESIGN, founder order 2026-09-26 ("I don't want exercise names list to be an option or show at all", "I want the user to be able to select their Top Lift", "Use styles from the rest of the app and none of the pill nonsense", "if I can move it up down left or right it will show better") — LANDED `28ae0332`** (lead, hands-on; register D209; preview drawn by the real renderer https://claude.ai/artifact/6K1PGc7mURYkV6Gp7R9wnc): the card in the app's own style (Inter and Inter Display loaded into Skia, flat near-black ground, section labels, hairlines, one amber hero; no pills, glow, frame, trophy or icons); no exercise names anywhere on the image (the untitled-session title is the time of day); the top lift chosen from one row per exercise, opening on the heaviest lift with a new best today, else the heaviest, with "Don't show a top lift"; the PR pills replaced by the same row-and-list; a new photo opens a move-and-zoom view with the real card over it, and the framing is drawn identically by preview, export and thumbnails; over a photo the title sits top and the numbers bottom. Files: `src/lib/shareCard/drawShareCard.js`, `cardTypefaces.js`, `photoFraming.js`, `src/components/SharePhotoFramer.js`, `src/screens/ShareCardScreen.js`, `src/screens/WorkoutSummaryScreen.js` (lift options, the session's own date), `src/lib/sessionShareData.js`, `src/components/BeforeAfterShareSheet.js` (fonts only), `scripts/render-share-card.cjs`, tests. Founder questions open: which of the six researched additions to build (in chat). Device checklist in chat.

**TODAY: THE WEIGH-IN BOX SITS UNDER THE COMMUNITY ROW, founder order 2026-09-26 — LANDED `b6e1140c`** (on main): the weigh-in box that disappears once you log moved to sit directly under the Community "nobody you follow" row; everything else on Today keeps its place.

**RECOVERY GETS ITS OWN PLACE, founder question 2026-09-26 ("Should we have a place in Progress exclusively for recovery ... it looks like a good feature now hard to find") — LANDED `1ef366ef`** (lead, hands-on; register D208; preview https://claude.ai/artifact/CmVbzKKSj6JbzqmmqDiQb7): a Recovery row in the Progress top card after the existing rows opens a new Recovery screen holding the section exactly as it stood, same order, first heading "Your ratings"; Consistency keeps the sessions milestone. Files: `src/screens/RecoveryScreen.js`, `src/screens/AnalyticsScreen.js`, `src/screens/ConsistencyScreen.js`, `src/components/ReadinessCards.js`, `src/lib/recovery/recoveryPillar.js`, `src/navigation/RootNavigator.js`, their tests.

**COMMUNITY SHARING ON FOR EVERYONE, founder order 2026-09-26 ("Flip them all and I test with all ... It is on for all users by default. New and existing they can turn it off if they want after. We need that in the privacy policy also") — BUILT; MIGRATIONS 184 AND 185 APPLIED 2026-09-26 13:49 AND 13:50 UTC** (lead, hands-on; register D194 addendum 2): the privacy policy (in-app, the hosted pages, the Markdown source, the docs/web mirror) gains a Community section and loses its "no community" claim, with a guard (`9aa9cba0`); `migrate_184` (column default on, new-profile fallbacks on and minor-aware, `community_get_me` returns the setting) and `migrate_185` (flip every pre-22-September profile, today the founder's two test accounts) written with a line-by-line guard (`9aa9cba0`), both APPLIED 2026-09-26 under the founder's phrase given that day (184 at 13:49 UTC, 185 at 13:50 UTC, checksum protocol, both acceptance blocks passed; verified read-only: default on, both test profiles on/everyone, 175 community functions pinned and none anon-executable; apply record in the `supabase/README.md` status block, guard moved to the applied state); the phone mirrors the server's setting and the summary names a refused post (`5a7dfcc0`). The founder delegated the remaining product calls on 2026-09-26 ("Make the decisions that bring the absolute best product for end users ... I do not need to make these decisions"): the volyume.app deploy (not run since 1 September; also switches on four iOS app-link paths), an in-app policy-change notice, the under-18 default and the lawful basis are lead rulings under D33; the store data-safety answers stay founder-side (console access).

**SHARE IMAGE AMBER OUTLINE, founder 2026-09-26 ("there's no border or outline so no obvious ending on a share. Need the amber outline we have in other areas of the app")** — DONE: `drawShareCard` draws a square amber frame (the app's accent, 6 px at 1080 wide) on the image's edge, last, so it sits over a photo, on every card type and the fallback; the workout name about a fifth and the total about three tenths smaller ("a bit more fitting and elegant in size"); the logo 1.6 times larger and the footer on the same left and right edges as every other row ("The alignments look a bit off too"); share tests 149 pass.
**SHARE IMAGE: OUTLINE ROUND THE CONTENT, AND TOP LIFTS, founder 2026-09-26 ("that outline isn't the outline of the content it looks too high"; "Could we do Top Lifts? And they can select more than one if they'd fit? Top Lift if only one selected Top Lifts if they select more than one. They don't have to select any number it's the end users choice")** — DONE: the outline now goes round the content itself, top line to logo, rounded like the app's cards, with the footer following the content and the panel centred on the image (on a story, kept clear of the top 14% and bottom 20%), on every card type and the fallback (`layoutCard`, `frameBand`, `drawOutline` in `src/lib/shareCard/drawShareCard.js`); the share screen's lift picker is a tick list, any number or none, a lift with no room left on the image greyed out ("No room on this image"), the heading and the image label "Top lift" for one and "Top lifts" for more, and "Only N of your M lifts fit on this image." when a later change leaves less room; the count that fits is decided at the 1080 design width (`sessionLiftsThatFit`), so the preview and the saved image always agree.

**RECOVERY LEARNS EACH PERSON'S RECOVERY, founder question 2026-09-26 ("We had recovery intelligence that learns people's recovery and adjusts as it goes along based on performance from a start. Is that what you've used for the recovery section or have you used rudimentary numbers?") and order ("We ship all we don't hold things back ... we need an elegant way to show and demonstrate this intelligence too") — FIRST BUILD `04c92d66` WITHDRAWN AFTER ITS REVIEW (D210 addendum); THE REBUILD'S OWN REVIEW SAID DO NOT LAND, REDESIGNED (D210 addendum 3); THE SECOND REVIEW'S BLOCKER FIXED (D210 addendum 5: workout days, not comparisons, in the clarity test; full calibration 22 cases, at most 2 in 600 shown a direction wrongly; it now almost never moves anyone in twelve weeks); LANDING ON MAIN 2026-09-26; NO BUILD (founder: the founder runs builds)** (lead, hands-on; spec `docs/recovery-programme-2026-09-25/14-PERSONAL-LEARNING-V2.md` section 0 "As built"): one recovery speed per person, starting at the "How's your recovery?" answer, learned from the same lift on the same day of the week (continuous outcome, bounded fit, per-day drift, outlier guard, straight sets only, lifts logged as planned left out, injury limits and return periods excluded), moved only past a gate set by the full calibration on a harsher simulation (`PERSONAL_LR_MIN` 10, 600 simulated athletes a cell with weekday effects, breaks and prescribed-reps logging: a direction shown to at most 2.2% of people whose recovery equals the start, the wrong one to at most 0.7%). Reach, stated: on a varied schedule without a plan it finds 123 of 600 slow recoverers and 11 of 600 fast ones in twelve weeks; on fixed weekly schedules and for plan users it found none, and those people see "Not learning yet" or "Still learning" with the reason (the plan-user reach goes to the founder with the numbers). Memo in `load.js` keyed on everything the learner reads, injury limits applied first; 4 to 5 ms per person in node for the heaviest history (33 ms under Jest, was 80), exact to the previous code bit for bit, pinned by an operation count. Shown by the "Your recovery speed" card under Recovery by muscle and in the caption and each muscle's breakdown once in use; a 2.3.0 What's New line. Recovery and card tests 14 suites, 292 tests at the redesign; the full calibration run passed (27 tests, 259 s). RECOVERY PATH for the second review: if the review agent dies, relaunch it with the same brief over the branch commits; nothing lands on main before its findings are dealt with and the full gate. Then the store set is delivered (task below).

**WELCOME CAPTURES RENDERED FROM THE CURRENT APP, founder 2026-09-26 ("I did not create the screenshots. Claudecode rendereed then itself") — DONE ON THE BRANCH** (lead, hands-on; register D211): the paper-render harness gained a Welcome pass (phone-height layout with the real tab bar, clean account state through the offers' own dismissal keys), real Skia rings (recorded paths drawn as SVG) and meal-time food logs; `welcome.js` shoots and encodes the three captures at 480 x 940; `assets/welcome/` replaced. STORE LISTING SET: the harness now renders it (`4d3b8274`: nine captures at Play 1080 x 1920 and App Store 1290 x 2796, PNG, into scratch; Sonnet lane, lead-reviewed). The first set's Recovery capture opened on three "N/A" dials (the persona had no ratings). v2 BUILT (lead, hands-on, lands with the recovery learning): the persona gains rated sessions and eleven weeks of history before its block, generated by the recovery model itself at a faster-than-average speed (0.75, 12% sensitivity), which the learner then finds on its own through the app's loader (the store pass asserts it; under the redesigned learner 110 comparisons, "Faster than first estimated"); a phone shot can start at a named section (`scrollToText`), so Recovery is shot twice (the muscle figure, and the "Your recovery speed" card); and a converter bug that drew the body figure's back view on top of its front (a `<G x>` a browser ignores) is fixed. Ten captures a platform; Play takes eight, so the founder picks. Delivered to the founder once on main; the uploads are founder-side.

**COMMUNITY OPEN QUESTIONS RULED, AND A SHARING DEFECT FIXED (register D212, lead, 2026-09-26, under the founder's delegation)** — volyume.app DEPLOYED (run #81, verified live: free wording, the Community privacy section, the link pages; the iOS app-link paths inert, no entitlement); the in-app policy notice is 2.3.0's first What's New line; under-18s start with "Share what I did" OFF (ICO Children's Code standard 7); lawful basis as the policy states. DEFECT: the Join screen omitted the sharing choice on create and published it only when on, so since 184 an "off" at Join would have been stored on (no profile created in the window). FIXED ON THE BRANCH: the choice travels with the create; migration 186 (an omitted choice on a new profile is off again) APPLIED 2026-09-26 18:44 UTC under the founder's "Run all against production" (supabase/README). Founder-side: the store data-safety answers (Play: written out in `docs/PLAY_STORE_LISTING.md`, re-derived from the code 2026-09-26; App Store: a Sonnet read lane maps the same list onto Apple's categories, checking each item in the code, into the scratchpad for delivery in chat; recovery path: relaunch it with the same brief, it writes nothing in the repo).

**NUTRITION HEADER GAP, founder device report 2026-09-26 ("The Nutrition and Trends button is almost touching the content below") — FIXED ON MAIN `67191120`** (lead, hands-on; gate over the commit in an isolated worktree `lint clean, tsc clean, check:imports OK over 2091 files, jest 1377 suites passed and 1 skipped, 21585 tests passed and 16 skipped, none failed`): Nutrition was the only tab root with no gap under ScreenHeader (its blocks use margins, not a content gap); the day pager now carries the spacing.lg the other tabs put there, and `tabHeaderGap.guard.test.js` pins it against Today, Train and Coach. Files: `src/screens/DiaryScreen.js` and the guard.
...
**VERSION 2.4.0 (2026-09-26)** — set on the founder's instruction ("Bump the version also as 2.3.0 has been burned. Use 2.4.0"): `app.json`, `package.json`, `package-lock.json`; buildNumber and versionCode untouched; the 2.3.0 What's New notes carried to 2.4.0 whole (the sheet shows notes for the exact running version), the share line now "lifts". Founder-side: create 2.4.0 in App Store Connect before the next iOS upload. Before it: **VERSION 2.3.0 (2026-09-26)**, set on the founder's "Just bump to 2.3.0", burned the same day.
...
- **SESSION SHARING DEFAULT, founder reminder 2026-09-26 ("Remember the
  ...
  ruling 2), ONE GAP FOUND; CLOSED 2026-09-26: the mirror landed
  (`5a7dfcc0`) and migrations 184 and 185 were applied 13:49 and 13:50 UTC
  under the founder's phrase (section 1, Community sharing line).** Observed: the client default
