...
**SHARE IMAGE AMBER OUTLINE, founder 2026-09-26 ("there's no border or outline so no obvious ending on a share. Need the amber outline we have in other areas of the app")** — DONE: `drawShareCard` draws a square amber frame (the app's accent, 6 px at 1080 wide) on the image's edge, last, so it sits over a photo, on every card type and the fallback; share tests 149 pass.
