...
| D3 | In-the-moment capture creates the episode | LANDED | W4B T2-11 preselect capture (9d5b4a1b); round 14 R14-2 closed the round-13 QUALIFIED half: the session in progress reflects a mid-session capture on return-focus (previously stale until exercise change or the swap sheet) |
...
| H2 | Erasure + export reach capability tables | LANDED (round-1 QUALIFIED Q4, fixed) | the portability export now carries effective_choice and adaptation_mode - both user-supplied decisions; round 12 R12-4 closed: a deleted COMPLETED session's effects record is tombstoned (it previously stayed live in the export and sync), and the tombstone survives later writes; round 13 R13-3 closed the THIRD delete path: Clear workout history tombstones every record with the sessions (driven) - erasure now reaches all three paths |
