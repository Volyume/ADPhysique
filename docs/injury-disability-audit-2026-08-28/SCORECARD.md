...
| H2 | Erasure + export reach capability tables | LANDED (round-1 QUALIFIED Q4, fixed) | the portability export now carries effective_choice and adaptation_mode - both user-supplied decisions; round 12 R12-4 closed: a deleted COMPLETED session's effects record is tombstoned (it previously stayed live in the export and sync), and the tombstone survives later writes |
...
| # | Variable | State | Evidence |
|---|---|---|---|
| L1 | Blank sessions / first-add / restore | LANDED | W1 L2; serveGuard; rounds 11-12 (R11-2/R12-3): all THREE ad-hoc constructions mint a stable slot id (build-a-workout, repeat-as-is, and the in-session picker add - the last found in round 12, where a "Start without a plan" session was entirely keyless), so the effects record keys per slot on every blank-session shape |
