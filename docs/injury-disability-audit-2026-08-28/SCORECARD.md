...
| # | Variable | State | Evidence |
|---|---|---|---|
| L1 | Blank sessions / first-add / restore | LANDED | W1 L2; serveGuard; rounds 11-13 (R11-2/R12-3/R13-1): four ad-hoc constructions were found and minted one by one (build-a-workout, repeat-as-is, the picker add, Home's repeat card) - and the round-13 chokepoint ends the enumeration: withSetsArrays mints for any keyless entry, so every blank-session shape, present and future, keys per slot |
