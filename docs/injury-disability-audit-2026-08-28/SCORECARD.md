...
| B6 | Post-session line + what-changed detail | LANDED | W3A T2-07/T2-22; round 10 closed the omitted lane (per-slot entries; performed omissions revoke); round 11 closed the SUBSTITUTED lane the same way (R11-1: performed-original substitutions revoke, a removed substitute converts to an omission, and the receipt reads toChosenByUser - "You chose X in for Y", neutral headline on any user-chosen slot) and the ad-hoc identity gap (R11-2); the residual condition is narrow and stated: an omission re-added but NOT trained stands, correctly, and an untrained-but-standing substitution keeps describing the served session |
...
| # | Variable | State | Evidence |
|---|---|---|---|
| L1 | Blank sessions / first-add / restore | LANDED | W1 L2; serveGuard; round 11 (R11-2): the two ad-hoc entry points mint a stable slot id at construction, so the effects record keys per slot there too - the round-10 per-exercise collapse no longer survives on build-a-workout or repeat-as-is sessions |
