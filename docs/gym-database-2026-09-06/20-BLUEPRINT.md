...
## GD-25 Name sanity bounds (lead ruling on the second build, 2026-09-07)

Observed in the second build: the Third Space operator adapter carried a
whole club page into one venue name (over 2,000 characters); composed
names repeated the brand ("The Gym Group The Gym Health And Fitness St
Helens College") or carried the outward code as a word ("Third Space
Tower Bridge Se1"). Rulings: a source name longer than 80 characters or
more than 8 tokens is rejected at normalisation and the record falls
back to brand plus town (from the address or ONSPD), flagged
`name_source = 'fallback'`; a branch name whose tokens already include
the brand's own tokens is used as-is rather than prefixed; a trailing
token that equals the venue's outward code is removed from the name
(the outward lives in its own field); an operator adapter that yields
any rejected name is reported per operator in the audit so the adapter,
not the name rule, is fixed next. The seed folder
`supabase/seed_gyms_v1/` is generated, not committed: it is gitignored
and regenerated by `node scripts/gyms/seed-sql.mjs` at apply time from
the committed canonical data, so the repository record is the data plus
the deterministic generator.

## GD-26 Town casing and operator-feed absence (lead ruling on the third build, 2026-09-07)

Observed in the third build: towns from Active Places stay all-caps
("WOLVERHAMPTON", "LETCHWORTH GARDEN CITY") while names are now cased;
a venue known only to Active Places can carry a bare brand name
("Fitness First", Dundee) for a club the operator's own current feed
does not list. Rulings: GD-18 casing applies to `town` and
`address_line` as well as names. A bare brand name (name equals the
brand or one of its aliases) is displayed as brand plus town. When an
operator feed was acquired with zero or near-zero failures (the
manifest records it), a venue carrying that brand from another source
alone and not matched to any feed row keeps `status` but gets
`verification_status = 'operator_unconfirmed'` and a `needs_review`
