...
| AC-2 | ALL-THREE → VERIFIED | ChatGPT Q2; Gemini Q2; Claude Q2 | Frontier split: nutrition coaches adjust nutrition only; training engines adjust training only; none close both | CONFIRMED YES (Volyume closes both) — nutrition `weeklyCoach.js:773-785`; training `:176-191`; steps `:873-883`; cardio `:913-914` |
...
| AC-5 | TWO | ChatGPT Q6; Claude Q-LLM | Deterministic, manual (no-LLM) coaching is a trust asset; LLM carries hallucination/ED risk | CONFIRMED YES — fully deterministic, no LLM (CLAUDE.md; `pass1-section2-engine-rules.md`) |
...
| AC-8 | TWO | ChatGPT; Claude Q-algo-vs-human | Human-coach hybrid (Caliber, Future) is the premium alternative | CONFIRMED NO (deliberate: deterministic engine, no human coach) → Q-AC2 |
