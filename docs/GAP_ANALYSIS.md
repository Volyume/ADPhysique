...
| 19 | Drop-set / myo-rep / rest-pause / AMRAP picker | **Picker done 2026-05-29.** All four now appear in the active-workout set-type bottom sheet (`SET_TYPE_OPTIONS`) with plain-language labels + descriptions, and the selected type shows its name on the set row (`SET_TYPE_DISPLAY` / SetEntry `SET_TYPE_LABELS` no longer collapse them to "Working"). Schema/engine already supported these `set_type` values (counting, AMRAP `is_amrap` wiring), so no migration. **Remaining: the cluster banner** (activation set + mini-set counter + "Cluster complete" button for myo-reps / rest-pause) — a focused live-workout interaction, deliberately not rushed into ActiveWorkoutScreen at the tail of a long session. | picker done; cluster banner remaining | Claude |
...
| 25 | 3 v1.1 features in FEATURE_MAP | **Partially shipped.** Refeed = row 7 (done). Body comp deep: BF% input + trend = row 2 (done 2026-05-29); trend *smoothing* (EWMA on BF%) and delta-since-photo remain (delta-since-photo is blocked on the photo timeline, deferred per `BUDGET_POSTURE_LOCKED.md`). Share-pack PDF = extend ShareCardScreen to PDF export (not started). | M (remaining: share-pack PDF + BF% smoothing) | Claude |
