...
| 12 | Sync-layer rework (full-resync cost + legacy drift) | **In progress 2026-05-29.** Discovery: migrate_012 already added `updated_at` + `(user_id, updated_at DESC)` indexes to every heavy table, so the full-resync fix (12b) needs NO migration, only a client watermark. **Workouts done:** `pullFromCloud` now reads a per-table watermark (`lib/sync/watermark.js`, 11 tests) and asks the cloud only for workouts changed since the last pull; cursor advances only on a clean pass and is cleared on sign-out (so sign-in still full-pulls), making it self-healing. **Remaining (each its own commit):** apply the same watermark to routines/routine_exercises, mesocycles/mesocycle_weeks, programmes, and the profile/notes tables; then the 12a hygiene (migrate consumers off the legacy helpers). **Needs device verification** before extending: incremental-sync semantics can't be exercised by the in-repo suite (no two-device/offline harness, deferred per BUDGET_POSTURE_LOCKED). | in progress (workouts) | Claude |
