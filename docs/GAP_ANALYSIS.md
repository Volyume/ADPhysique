...
| 28 | Search subnav tabs | **Decisions locked 2026-05-28:** (a) **5 tabs** per `UI_FLOWS_LOCKED.md` (Recents / Favourites / Frequents / Custom / Database); (b) Frequents = **full server pipeline**. Recents/Favourites/Custom/Database data sources already exist. Frequents pipeline has clear precedents: cloud table + nightly `cron.schedule` worker mirroring migration 031 (cascade workers, pg_cron), food sync-RPC + pull handler mirroring migration 016 + `src/lib/sync/tables/*`, local cache table in `database.js` SCHEMA_MIGRATIONS, client reads the local cache. Migration is founder-applied (Rule 6) and can't be run/verified from the sandbox, so this is the next focused build (runtime-critical: new synced table + cron). | next | Claude |
