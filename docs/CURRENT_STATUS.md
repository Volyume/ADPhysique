...
| 10 | ~~**Food preferences: dislike / exclude list.**~~ **Code complete 2026-05-27.** Shared `kind` column on `food_favourites` (values `'fav'` / `'dislike'`) carries both states in one table. Helpers: `setFoodPreference`, `cycleFoodPreference`, `getFoodPreference`, `getDislikes`, plus a `toggleFavourite` back-compat shim. UI: `FoodRow` shows a star for fav, a struck-through name + cross-circle for dislike. `FoodSearchScreen` long-press cycles `none → fav → dislike → none` and renders a collapsible "Excluded" section listing every disliked food. Sync handler at `foodDomain.js` ships `kind` in `food_sync_push` and reads it in `applyFavouriteFromCloud` (pre-mig-048 rows default to `'fav'`). Cloud migration 048 adds `kind text NOT NULL DEFAULT 'fav'` + CHECK constraint to `food_favourites`. Old AAB compat: legacy rows without `kind` default to `'fav'` server-side; old client never sends `kind` and is unaffected. 13 new Jest assertions added; full suite 1726/3/0. Founder action: apply migration 048 via Supabase Dashboard. Coach-side filter (skip disliked foods when meal suggestions ship) is future-wired but inert until a meal suggester exists. | none formal | done | Claude |
| 11 | ~~**Recipe builder UI.**~~ **Shipped 2026-05-27** (commit `61636ee`). `MyRecipesScreen` (list, create, delete) + `RecipeBuilderScreen` (name, servings, notes, ordered ingredient list, live total + per-serving macros preview). Ingredient picking reuses `FoodSearchScreen` in `pickMode:'recipe'`. CRUD helpers in `src/lib/food/db.js` with atomic ingredient replacement via `withTransactionAsync`. 16 new Jest assertions; suite 1742/3/0. No new migration. | `UI_FLOWS_LOCKED.md`, schema in `migrate_015_food_logging.sql` | done | Claude |
...
4. **Apply migration 048** (`supabase/migrate_048_food_preferences_kind.sql`). Adds the `kind` column to `food_favourites` that the new dislike feature ships against. Old AAB compatible (DEFAULT 'fav' fills legacy rows). Verification query in `supabase/README.md` § Verify food_favourites.kind.
5. **Tear down the throwaway `volyume-e2e-test` Supabase project** + delete the four `SUPABASE_TEST_*` repo secrets in GitHub Actions. Originally set up for T7/T8 live-cloud E2E; that suite was deleted 2026-05-27 as out of scope for the product (see § 0 today). Nothing in the repo references those secrets any more.
6. **Close PR #5 without merging.** It was opened today to read CI check status through MCP; the work it contained was already merged to main, then reverted. Safe to close.
7. **Point `volyume.app` DNS at the GitHub Pages site** so the in-app `/privacy` link resolves. The file (`public/privacy/index.html`) and deploy workflow are already shipped. This was the only remaining piece for the privacy URL.
8. (Optional, low priority) Add `EXPO_PUBLIC_USDA_API_KEY` repo secret if USDA fallback is wanted active.
