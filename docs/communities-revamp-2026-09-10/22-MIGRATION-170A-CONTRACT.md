...
unchanged** (same contract `styles` already has); send an array, including `[]`, to replace it. Invalid key or
>3 elements → `invalid_input`. No new consent type: rides the existing `community_visibility` consent this RPC
already records.
...
`invalid_input`. **Every caller must be updated to pass `_today`.**
...
`_today` required (`YYYY-MM-DD`, same rule as above). Rate-railed, 120/hour (same rail as
...
never a minor, up to 3, trained-today preferred. One call for the whole Hub instead of one per cohort
