...
- IN FLIGHT (agents, opus, read-only; WIDER WINDOW under the founder's continue-without-checkpointing order): Phases 39-43 → AUDIT-CLOCK-SCALE-LOCAL-PARTNER.md; Phases 45-46+48-49 → AUDIT-MATURITY-AND-SIX-MONTHS.md; Reviews A-E → REVIEW-A-six-month.md / REVIEW-B-returning.md / REVIEW-C-sync.md / REVIEW-D-truth.md / REVIEW-E-relationship.md (all five fresh opus agents, exact commissioned questions, E on the twelve verbatim). Recovery per lane: partial file lead-reviewed against the phase text, relaunched with the same brief. Recovery: partial file lead-reviewed against the order's phase text, lane relaunched with the same brief; agents write only their own file.
