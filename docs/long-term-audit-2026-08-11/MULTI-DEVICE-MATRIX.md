...
| A applies weekly coaching | B's copy of the week's output lacks the receipt → double Apply | Receipt propagates: one deterministic output id per week (v71/v72), LWW receipt preservation on pull; B's Apply goes dead on its next pull | S-16/S-22; corrected 135 completes cloud convergence | reinstall E2E test 7; coachOutputReid suite |
...
| A chooses adjustments (seeds next block) | B's stale mesocycle push nulls the ledger / duplicates learning | Ledger key omitted from push when absent (can't null); blockLedgerRunner idempotent by version (no duplicate learning) | S-18, S-22 CLEAN | adaptiveBlock/e2e suites |
