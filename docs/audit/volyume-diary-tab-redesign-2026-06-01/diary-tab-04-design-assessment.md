...
| Training context | Pre/Post-workout exist but sit in a wellness frame (`DiaryScreen.js:40-47`) | A physique app should treat peri-workout meals as first-class named meals the user logs whenever they train (Phase 3 section 4). NOT a scheduled training-day model: people train when they want, so the app must never gate or assume a day type | Pre/Post are generic slots, not first-class meals in a flexible model; the value is naming them, not detecting training days | Medium |
