...
| ~~**A2-019**~~ | ~~Sign-out's `AsyncStorage.clear()` wipes device-wide prefs~~ | **RETRACTED 2026-05-31.** Conflicts with the hard lock. `IDENTITY_AND_OWNERSHIP_LOCKED.md` § C lines 80-83 specify sign-out calls `AsyncStorage.clear()` with "no carve-outs for accessibility prefs, crash log, install counters, or anything else. Same hammer as delete-account." Code (`useAppStore.js:278-283`) matches that founder direction. `AsyncStorage.clear()` is app-sandbox scoped, so it cannot touch other apps, and local SQLite is already wiped, so no other-account caches remain to leak. Current behaviour is correct; no change. |
