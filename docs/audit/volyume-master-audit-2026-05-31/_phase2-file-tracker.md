# Phase 2 — per-file read tracker

Status: `[x]` read in full + findings recorded · `[~]` partially read (see 02-code-audit.md scope notes) · `[ ]` not yet read.
Generated 2026-05-31T16:09Z. Total non-test source files: 227 (83,854 LOC).
Read so far: 226 files. PHASE 2 COMPLETE.

## App.js/


## src/components/

- [x] `AnimatedEntrance.js` (53)
- [x] `BackHeader.js` (70)
- [x] `BlockProgressCard.js` (108)
- [x] `BodyDiagramHeatmap.js` (332)
- [x] `BottomSheet.js` (136)
- [x] `BrandMark.js` (85)
- [x] `Button.js` (108)
- [x] `Card.js` (86)
- [x] `Chip.js` (72)
- [x] `DifferentialBadge.js` (115)
- [x] `EmptyState.js` (129)
- [x] `EngineLog.js` (161)
- [x] `ExerciseCard.js` (126)
- [x] `FatigueTrendCard.js` (86)
- [x] `FeedbackSheet.js` (486)
- [x] `GradientCard.js` (40)
- [x] `Illustrations.js` (167)
- [x] `InfoTooltip.js` (58)
- [x] `PRCelebration.js` (280)
- [x] `PeekMenu.js` (224)
- [x] `PlateCalculator.js` (247)
- [x] `PressableCard.js` (85)
- [x] `ProGate.js` (252)
- [x] `ReadinessCards.js` (305)
- [x] `RestTimer.js` (324)
- [x] `ScreenHeader.js` (73)
- [x] `SearchBar.js` (77)
- [x] `SetEntry.js` (334)
- [x] `Skeleton.js` (101)
- [x] `Sparkline.js` (64)
- [x] `Stepper.js` (83)
- [x] `StepsCard.js` (108)
- [x] `SvgBarSparkline.js` (84)
- [x] `SvgLineChart.js` (190)
- [x] `TierComparisonStrip.js` (119)
- [x] `Toast.js` (242)
- [x] `VolumeBars.js` (85)
- [x] `WhatsNewSheet.js` (176)

## src/components/food/

- [x] `EmptyDiary.js` (41)
- [x] `EntryRow.js` (140)
- [x] `FoodDetailSheet.js` (271)
- [x] `FoodRow.js` (99)
- [x] `HeldDecisionCard.js` (113)
- [x] `MacroBreakdownSheet.js` (119)
- [x] `MacroRings.js` (250)
- [x] `MealSection.js` (58)
- [x] `QuickAddSheet.js` (183)
- [x] `ServingPicker.js` (110)
- [x] `SourceChip.js` (49)

## src/lib/

- [x] `accessibilityPrefs.js` (14)
- [x] `activitySteps.js` (175)
- [x] `algorithms.js` (1100)
- [x] `blockAdvisor.js` (385)
- [x] `chartGeometry.js` (100)
- [x] `clusterSet.js` (83)
- [x] `coachApply.js` (261)
- [x] `coachingGoals.js` (513)
- [x] `cyclePrefs.js` (39)
- [x] `dailyNarrative.js` (135)
- [x] `dataBackup.js` (132)
- [x] `database.js` (5574)
- [x] `differentialPaywall.js` (181)
- [x] `edPatternDetector.js` (146)
- [x] `engineTelemetry.js` (11)
- [x] `errorLog.js` (307)
- [x] `exerciseDisplay.js` (117)
- [x] `exerciseMetadata.js` (242)
- [x] `feedback.js` (238)
- [x] `formTips.js` (196)
- [x] `haptics.js` (80)
- [x] `health.js` (586)
- [x] `importExternal.js` (488)
- [x] `insightsEngine.js` (232)
- [x] `liftProgress.js` (98)
- [x] `links.js` (26)
- [x] `mesocycle.js` (472)
- [x] `nutritionEngine.js` (860)
- [x] `observability.js` (721)
- [x] `planAutoGen.js` (197)
- [x] `planEngine.js` (1549)
- [x] `planSwitch.js` (53)
- [x] `poolGenerator.js` (131)
- [x] `proGate.js` (61)
- [x] `recoveryEMA.js` (114)
- [x] `restSound.js` (188)
- [x] `seedExercises.js` (985)
- [x] `seedRoutines.js` (1568)
- [x] `sentry.js` (192)
- [x] `stepsLaunchPrompt.js` (143)
- [x] `stepsSummary.js` (30)
- [x] `storeReview.js` (40)
- [x] `strengthStandards.js` (132)
- [x] `supabase.js` (187)
- [x] `swapEngine.js` (300)
- [x] `sync.js` (1732)
- [x] `syncQueue.js` (225)
- [x] `travelMode.js` (295)
- [x] `unilateral.js` (77)
- [x] `units.js` (121)
- [x] `weeklyCoach.js` (1093)
- [x] `wellbeing.js` (36)
- [x] `whyThisTemplates.js` (419)

## src/lib/food/

- [x] `bulkEntryOps.js` (58)
- [x] `csvExport.js` (94)
- [x] `curatedFoods.js` (124)
- [x] `curatedMeals.js` (238)
- [x] `db.js` (1112)
- [x] `frequents.js` (38)
- [x] `libraryDelta.js` (243)
- [x] `mealSuggest.js` (197)
- [x] `ocr.js` (61)
- [x] `ocrParser.js` (147)
- [x] `sanityChecks.js` (92)
- [x] `searchTabs.js` (26)
- [x] `seed.js` (257)
- [x] `waterfall.js` (167)
- [x] `writeback.js` (158)

## src/lib/food/normalisers/

- [x] `usdaToFood.js` (65)

## src/lib/food/sources/

- [x] `liveOff.js` (106)
- [x] `localCache.js` (193)
- [x] `usda.js` (81)

## src/lib/notifications/

- [x] `activeWorkout.js` (197)
- [x] `categories.js` (93)
- [x] `channels.js` (41)
- [x] `handler.js` (85)
- [x] `index.js` (81)
- [x] `listeners.js` (101)
- [x] `permissions.js` (51)
- [x] `preferences.js` (287)
- [x] `pushToken.js` (163)
- [x] `quietHours.js` (124)
- [x] `scheduler.js` (428)
- [x] `telemetry.js` (105)
- [x] `trainingReminders.js` (178)

## src/lib/observability/

- [x] `sentryScrub.js` (229)

## src/lib/payments/

- [x] `cascade.js` (267)
- [x] `catalogue.js` (84)
- [x] `index.js` (14)
- [x] `playBilling.js` (339)
- [x] `restore.js` (67)

## src/lib/sync/

- [x] `conflict.js` (102)
- [x] `index.js` (39)
- [x] `queue.js` (150)
- [x] `registry.js` (188)
- [x] `runner.js` (242)
- [x] `telemetry.js` (86)
- [x] `transport.js` (178)
- [x] `watermark.js` (85)

## src/lib/sync/tables/

- [x] `bodyComposition.js` (152)
- [x] `dailySteps.js` (125)
- [x] `edPatternFlags.js` (50)
- [x] `foodDomain.js` (400)
- [x] `notificationPreferences.js` (149)
- [x] `nutritionTargets.js` (89)
- [x] `profiles.js` (203)
- [x] `recipeIngredients.js` (135)
- [x] `tierHistory.js` (52)
- [x] `weeklyCheckins.js` (140)
- [x] `weightLog.js` (24)

## src/lib/telemetry/

- [x] `events.js` (110)
- [x] `index.js` (38)
- [x] `sentryBridge.js` (34)
- [x] `transport.js` (99)

## src/navigation/

- [x] `RootNavigator.js` (1065)

## src/screens/

- [x] `ActiveWorkoutScreen.js` (2560)
- [x] `AddCustomFoodScreen.js` (267)
- [x] `AnalyticsScreen.js` (1436)
- [x] `Article9ConsentScreen.js` (271)
- [x] `BlockReflectionScreen.js` (305)
- [x] `BodyMetricsScreen.js` (1164)
- [x] `BuildWorkoutScreen.js` (614)
- [x] `CascadeGateScreen.js` (272)
- [x] `CoachHeldHistoryScreen.js` (259)
- [x] `CoachOutputScreen.js` (2110)
- [x] `CoachReviewScreen.js` (810)
- [x] `CoachingRemindersScreen.js` (339)
- [x] `CreditsScreen.js` (132)
- [x] `DebugLogScreen.js` (214)
- [x] `DiaryScreen.js` (776)
- [x] `ExerciseDetailScreen.js` (1076)
- [x] `ExerciseLibraryScreen.js` (713)
- [x] `FirstRunScreen.js` (127)
- [x] `FoodInsightsScreen.js` (279)
- [x] `FoodSearchScreen.js` (837)
- [x] `GoalChangeSummaryScreen.js` (354)
- [x] `GoalLockConsentScreen.js` (203)
- [x] `HomeScreen.js` (2344)
- [x] `ImportScreen.js` (477)
- [x] `LiftProgressScreen.js` (141)
- [x] `LoginScreen.js` (588)
- [x] `ManualBuilderScreen.js` (1250)
- [x] `MesocycleBuilderScreen.js` (548)
- [x] `MyMealsScreen.js` (231)
- [x] `MyRecipesScreen.js` (205)
- [x] `NotificationSettingsScreen.js` (939)
- [x] `NutritionEducationScreen.js` (294)
- [x] `NutritionTargetsScreen.js` (1716)
- [x] `OnboardingScreen.js` (366)
- [x] `PRWallScreen.js` (590)
- [x] `PaywallScreen.js` (155)
- [x] `PlanDetailScreen.js` (430)
- [x] `PlanLibraryScreen.js` (791)
- [x] `PlansScreen.js` (1024)
- [x] `PrivacyPolicyScreen.js` (122)
- [x] `ProGoalSetupScreen.js` (726)
- [x] `ProOnboardingScreen.js` (1378)
- [x] `ProSetupCompleteScreen.js` (417)
- [x] `ProUpgradeScreen.js` (491)
- [x] `RecipeBuilderScreen.js` (334)
- [x] `RoutineDetailScreen.js` (865)
- [x] `ScanBarcodeScreen.js` (276)
- [x] `ScanLabelScreen.js` (307)
- [x] `SettingsScreen.js` (1435)
- [x] `ShareCardScreen.js` (1494)
- [x] `SubscriptionPolicyScreen.js` (193)
- [x] `SubscriptionScreen.js` (222)
- [x] `VolumeHeatmapScreen.js` (631)
- [x] `WeeklyCheckInScreen.js` (1300)
- [x] `WelcomeScreen.js` (277)
- [x] `WellbeingCheckScreen.js` (176)
- [x] `WorkoutHistoryScreen.js` (903)
- [x] `WorkoutSummaryScreen.js` (1305)
- [x] `YearOfLiftsScreen.js` (516)
- [x] `YouScreen.js` (226)

## src/store/

- [x] `useAppStore.js` (910)

## src/styles/

- [x] `theme.js` (365)

