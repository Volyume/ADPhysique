 ...
 *
 * Approach: DETERMINISTIC frame-by-frame. showcase-reel.html exposes
...
async function main() {
  ...
  const outPath = path.resolve(outArg || path.join(__dirname, 'out/showcase.mp4'));
  ...
  try {
    ...
    const templateUrl = 'file://' + path.join(__dirname, 'showcase-reel.html');
